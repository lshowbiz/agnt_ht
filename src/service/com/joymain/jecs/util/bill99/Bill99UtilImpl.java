...
/*		if(Bill99Constants.signType1.equals(jfi99billLog.getSignType())){
		...
		jfi99billLog.setDataType("1");//数据来源，1：PC
	...
	private String getSignMsg(Bill99 bill99){
		...
		if(Bill99Constants.signType1.equals(bill99.getSignType())){
			...
			if(flag==0){
				signMsgVal=appendParam(signMsgVal,"key",Bill99Constants.account.get(this.getMemberCodeByMoIdOrUserCode(userCode,flag)).get("key"));//账户编号
			}else{
				signMsgVal=appendParam(signMsgVal,"key",Bill99Constants.account.get(this.getMemberCodeByMoIdOrUserCode(bill99.getOrderId(),flag)).get("key"));//账户编号
			}
	...
	public Bill99 getBill99(Bill99 bill99,int flag ){
		...
		String[] ext2 = bill99.getExt2().split(",");
		String userCode = ext2[0];
		//bill99.setMerchantAcctId(Bill99Constants.account.get(this.getMemberCodeByUserCode(userCode)).get("memberCode"));//账户编号
		if(flag==0){
			bill99.setMerchantAcctId(Bill99Constants.account.get(this.getMemberCodeByMoIdOrUserCode(userCode,flag)).get("memberCode"));//账户编号
		}else{
			bill99.setMerchantAcctId(Bill99Constants.account.get(this.getMemberCodeByMoIdOrUserCode(bill99.getOrderId(),flag)).get("memberCode"));//账户编号
		}
	...
	/**
	 * 生成Bill99对象的通用方法(目前支持慈善、酒业)
	 * flag(2:慈善基金订单, 3:酒业配置单)
	 * 石义宏2013/12/10
	 */
	public Bill99 getObjectBill99(Bill99 bill99, int flag){
		bill99.setInputCharset(Bill99Constants.inputCharset);
		bill99.setPageUrl(Bill99Constants.pageUrl);
		bill99.setBgUrl(Bill99Constants.bgUrl);
		bill99.setVersion(Bill99Constants.version);
		...
		bill99.setExt1(getExt1MD5(bill99));
		bill99.setPostUrl(Bill99Constants.postUrl);
		bill99.setSignMsg(getObjectSignMsg(bill99));
		return bill99;
	}
	 ...
	 * @return
	 */	
	private String getObjectSignMsg(Bill99 bill99){
		//生成加密签名串
		///请务必按照如下顺序和规则组成加密串！
		String signMsgVal="";
		signMsgVal=appendParam(signMsgVal,"inputCharset",Bill99Constants.inputCharset);
