...
public class ChanjetUtilImpl implements ChanjetUtil {
	...
	private final Log log = LogFactory.getLog(ChanjetUtilImpl.class);
	...
	public static final String notifyurl = "http://test.joylifeglobal.net/jecsnew_vip/jfiPayChanjetReceive";//后台通知地址

    private SysIdManager sysIdManager;//生成ID号
...
//			signSb.append(chanjet.getPayerName());
	...
	public Chanjet getChanjet(Chanjet chanjet, int flag) throws UnsupportedEncodingException {
		...
		chanjet.setExpand(chanjet.getExpand()+","+flag);//存储：会员编号，标识（订单支付、充值）
		...
		chanjet.setOrderAmount((new BigDecimal(chanjet.getOrderAmount()).multiply(new BigDecimal(100))).toString());
	...
	public boolean checkJfiChanjetLog(HttpServletRequest request){
    	...
    	//MD5商户密钥
    	String key="&key=G08C17J21R90P3QF";//MD5商户密钥
    	signSb.append(key);
    	...
    	log.info("============畅捷支付接受的明文串=================="+plain);
    	log.info("============畅捷支付接收的MD5密文串=================="+signMsg);
    	log.info("============畅捷支付接收的MD5签名串=================="+sign);
