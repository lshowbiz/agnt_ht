 ...
 *
 */
public class JpoMemberOrderRenewalReportController extends BaseController
implements Controller{

	private static String[][] tableColHead=new String[][]{
    	{"1", "会员续约"},{"2", "占比"},{"3", "总收入"},{"4", "人数"},
    	{"5", "来自发展基金"}
    };
	private final Map<String,String> fmonthNameMap;
	public JpoMemberOrderRenewalReportController(){
	...
	public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if( StringUtils.isNotEmpty(search)){
			...
			if(StringUtils.isNotEmpty(fyear)){
				   sql=" select * from view_jpomemberorderrenewal where p_view_param.set_param('"+fyear+"')='"+fyear+"'";
			}else{
			...
			excelUtil.addString(wsheet, 0, row, fyear+""+LocaleUtil.getLocalText("jpomemberordertype.renewal"));
		...
		//	excelUtil.addString(wsheet, ++col, row , "总金额");
			...
			BigDecimal  renwalCount=null;
			BigDecimal  jjCount=null;
			...
			if(poMemberOrders!=null){
				...
				for(int i=0;i<poMemberOrders.size();i++){
					...
					amount=new BigDecimal(temp.get("amount").toString());//续约单总收入
					totalAmount=new BigDecimal(temp.get("totalamount").toString());//订单总收入
					proportion=amount.multiply(totalAmount);// 占比
					renwalCount=new BigDecimal(temp.get("totalnum").toString());//下续约单人数
					jjCount=new  BigDecimal(temp.get("jjnum").toString());//基金总 人数
					...
					excelUtil.addString(wsheet, col, ++row , renwalCount.toString() );
					excelUtil.addString(wsheet, col , ++row , jjCount.multiply(renwalCount).toString());
				}
		...
		return new ModelAndView("po/jpoMemberOrderRenewalReport");
	}
