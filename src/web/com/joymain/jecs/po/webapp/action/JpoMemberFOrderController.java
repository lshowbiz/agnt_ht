 ...
 *
 */
public class JpoMemberFOrderController extends BaseController implements Controller {
    ...
    throws Exception {
        ...
        if(!StringUtils.isEmpty(crm.getString("search","")) || !StringUtils.isEmpty(crm.getString("sysUser.userCode",""))){
        	if("C".equals(loginSysUser.getUserType())){
        		if(RequestUtil.saveOperationSession(request,"poMemberFOrders",3l)!=0){
        			return new ModelAndView("redirect:jpoMemberFOrders.html");
        		}
        	}else if("M".equals(loginSysUser.getUserType())){
        		if(RequestUtil.saveOperationSession(request,"poMemberFOrders",3l)!=0){
        			return new ModelAndView("redirect:jpoMemberFOrders.html");
		...
		if (RequestUtil.isMobileRequest(request)) {
			return new ModelAndView("mobile/po/jpoMemberFOrderList",
					Constants.JPOMEMBERORDER_LIST, jpoMemberOrders);
		}
        return new ModelAndView("po/jpoMemberFOrderList", Constants.JPOMEMBERORDER_LIST, jpoMemberOrders);
    }
}
