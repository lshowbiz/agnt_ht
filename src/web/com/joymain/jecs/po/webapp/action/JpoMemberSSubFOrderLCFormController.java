 ...
 *
 */
public class JpoMemberSSubFOrderLCFormController extends BaseFormController {/*
    ...
    public JpoMemberSSubFOrderLCFormController() {
    ...
    throws Exception {
        ...
        if ("deletePoMemberSSubFOrderLC".equals(strAction) || "editPoMemberSSubFOrderLC".equals(strAction) && !StringUtils.isEmpty(moId)){
    	...
    	if(loginSysUser.getIsMember()){
    		jpmProductSales = jpmProductSaleManager.getLCProductSales(loginSysUser.getCompanyCode(),Constants.SUBSTORE_FIRST_PURCHASE,"1");
    		
    	}else{
    		jpmProductSales = jpmProductSaleManager.getLCProductSales(loginSysUser.getCompanyCode(),Constants.SUBSTORE_FIRST_PURCHASE,"2");
    	}
    	...
    	while (its1.hasNext()) {
    		...
    		if("NLC010000000CN0".equals(jpoMemberOrderList.getJpmProductSale().getJpmProduct().getProductNo())){
				sumPrice3 = sumPrice3.add(jpoMemberOrderList.getJpmProductSale().getSubstoreFpPrice().multiply(new BigDecimal(jpoMemberOrderList.getQty())));
    		}
		}
		...
		request.setAttribute("fixeds", 0);
    	request.setAttribute("jpmProductSales", jpmProductSales);
    	request.setAttribute("pvamt", ConfigUtil.getConfigValue(loginSysUser.getCompanyCode().toUpperCase(), "store.f2.order.pvamt"));
        
    	if (RequestUtil.isMobileRequest(request)) {
			this.setFormView("/mobile/po/jpoMemberSSubFOrderLCForm");
		} else {
			this.setFormView("/po/jpoMemberSSubFOrderLCForm");
    ...
    *//**
    ...
    private boolean checkAmount(JpoMemberOrder jpoMemberOrder){
    	BigDecimal pvamt = new BigDecimal(ConfigUtil.getConfigValue(jpoMemberOrder.getSysUser().getCompanyCode().toUpperCase(), "store.f2.order.pvamt"));
    ...
    private int calcOrderAmount(HttpServletRequest request, Set jpoMemberOrderSet, Set jpoMemberOrderFeeSet, JpoMemberOrder jpoMemberOrder){
    	...
    	while (its1.hasNext()) {
			...
			if(!"LC".equals(jpoMemberOrderList.getProductType())){
    			...
    			isPass = true;
				sumPrice2 = sumPrice2.add(jpoMemberOrderList.getJpmProductSale().getSubstoreFpPrice().multiply(new BigDecimal(jpoMemberOrderList.getQty())));
				sumPV2 = sumPV2.add(jpoMemberOrderList.getJpmProductSale().getStoreFpPv().multiply(new BigDecimal(jpoMemberOrderList.getQty()))) ;
    		}
    		if("NLC010000000CN0".equals(jpoMemberOrderList.getJpmProductSale().getJpmProduct().getProductNo())){
    			isPass = true;
				sumPrice3 = sumPrice3.add(jpoMemberOrderList.getJpmProductSale().getSubstoreFpPrice().multiply(new BigDecimal(jpoMemberOrderList.getQty())));
    ...
    private Set fillInJfoMemberOrderList(HttpServletRequest request, JpoMemberOrder jpoMemberOrder){
		...
		for(int i_no = 0 ; i_no < g_no.length ; i_no++){
			if( !StringUtils.isEmpty(qty[i_no]) && !"0".equals(qty[i_no])){
				...
				if("2".equals(step)){
					jpoMemberOrderList.setPrice(jpmProductSale.getSubstoreFpPrice());
					jpoMemberOrderList.setPv(jpmProductSale.getSubstoreFpPv());
