 ...
 *
 */
public class JpoMemberSpecialUOrderFormController extends BaseFormController {
    ...
    public JpoMemberSpecialUOrderFormController() {
    ...
    throws Exception {
        ...
        if ("deletePoMemberSpecialUOrder".equals(strAction) || "editPoMemberSpecialUOrder".equals(strAction) && !StringUtils.isEmpty(moId)){
...
/*					CommonRecord crm = new CommonRecord();
    ...
    private boolean checkAmount(JpoMemberOrder jpoMemberOrder){
    	...
    	if(!"6".equals(jmiMember.getCardType())){
    		...
    		BigDecimal pv2=new BigDecimal(ConfigUtil.getConfigValue(jpoMemberOrder.getSysUser().getCompanyCode().toUpperCase(), "cardtype."+String.valueOf(cardType + 1)+".value"));
    		...
    		BigDecimal comparePv = new BigDecimal("0");
    		
    		if("2".equals(jmiMember.getMemberType()) && new BigDecimal("0").compareTo(jmiMember.getOriPv())==-1){
    			//(PV2 - PV1 - X + 35) / 2 = orderPv
    			comparePv = pv2.subtract(pv1).subtract(jmiMember.getOriPv()).add(new BigDecimal("35")).divide(new BigDecimal("2"));
    		}else{
    			comparePv = pv2.subtract(pv1).divide(new BigDecimal("2"));
    		}
    		
    		BigDecimal orderPv = jpoMemberOrder.getPvAmt();
    		
    		if("HK".equals(jpoMemberOrder.getCompanyCode())){
        		if("2".equals(jmiMember.getCardType())){
        			comparePv = comparePv.add(new BigDecimal("35"));
        		}
    		}
    		
    		if(comparePv.compareTo(orderPv)!=1){
        		return true;
        	}
    	}
    	return false;
	    	...
	    	if((curDate.after(startDate))&&(curDate.before(endDate))){
	    		...
	    		while (its1.hasNext()) {
	    			...
	    			if(jpoMemberOrderList.getPv().compareTo(new BigDecimal("0"))==1){
	    				jpoMemberOrderList.setPrice(jpoMemberOrderList.getJpmProductSaleTeamType().getPrice().multiply(new BigDecimal("0.95")));
	    			}
	    		}
	    	}
		}
