 ...
 *
 */
public class JpoMemberIROrderController extends BaseController implements Controller {
    ...
    throws Exception {
        ...
        if("C".equals(loginSysUser.getUserType())){
            RequestUtil.freshSession(request,"poMemberIROrders",3l);
        ...
        if(!StringUtils.isEmpty(crm.getString("search","")) || !StringUtils.isEmpty(crm.getString("sysUser.userCode",""))){
        	if("C".equals(loginSysUser.getUserType())){
        		if(RequestUtil.saveOperationSession(request,"jpoMemberIROrders",3l)!=0){
        			return new ModelAndView("redirect:jpoMemberIROrders.html");
        		}
        	}else if("M".equals(loginSysUser.getUserType())){
        		if(RequestUtil.saveOperationSession(request,"jpoMemberIROrders",3l)!=0){
        			return new ModelAndView("redirect:jpoMemberIROrders.html");
        ...
        return new ModelAndView("po/jpoMemberIROrderList", Constants.JPOMEMBERORDER_LIST, jpoMemberOrders);
    }
}
