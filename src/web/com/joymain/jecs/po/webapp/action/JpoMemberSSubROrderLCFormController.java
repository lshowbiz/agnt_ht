 ...
 *
 */
public class JpoMemberSSubROrderLCFormController extends BaseFormController {/*
    ...
    public JpoMemberSSubROrderLCFormController() {
    ...
    throws Exception {
        ...
        if ("deletePoMemberSSubROrderLC".equals(strAction) || "editPoMemberSSubROrderLC".equals(strAction) && !StringUtils.isEmpty(moId)){
    	...
    	if(loginSysUser.getIsMember()){
    		if("1".equals(loginSysUser.getJmiMember().getIsstore())){
    			jpmProductSales = jpmProductSaleManager.getLCProductSales(loginSysUser.getCompanyCode(),Constants.SUBSTORE_MAINTAIN_PURCHASE,"1");
    		}else if("2".equals(loginSysUser.getJmiMember().getIsstore())){
    			jpmProductSales = jpmProductSaleManager.getLCProductSales(loginSysUser.getCompanyCode(),Constants.SUBSTORE_MAINTAIN_PURCHASE,"1");
    		}
    	}else{
    		jpmProductSales = jpmProductSaleManager.getLCProductSales(loginSysUser.getCompanyCode(),Constants.SUBSTORE_MAINTAIN_PURCHASE,"2");
    	}
    	request.setAttribute("jpmProductSales", jpmProductSales);
        
    	if (RequestUtil.isMobileRequest(request)) {
			this.setFormView("/mobile/po/jpoMemberSSubROrderLCForm");
		} else {
			this.setFormView("/po/jpoMemberSSubROrderLCForm");
    ...
    throws Exception {
		...
		String key=null;
		String strAction = request.getParameter("strAction");
		if ("deletePoMemberSSubROrderLC".equals(strAction)) {
			...
			//if("addPoMemberSSubROrderLC".equals(strAction)){
				CommonRecord crm = new CommonRecord();
    ...
    private Set fillInJfoMemberOrderList(HttpServletRequest request, JpoMemberOrder jpoMemberOrder){
		...
		for(int i_no = 0 ; i_no < g_no.length ; i_no++){
			if( !StringUtils.isEmpty(qty[i_no]) && !"0".equals(qty[i_no])){
				...
				if("2".equals(step)){
					jpoMemberOrderList.setPrice(jpmProductSale.getSubstoreMpPrice());
					jpoMemberOrderList.setPv(jpmProductSale.getSubstoreMpPv());
