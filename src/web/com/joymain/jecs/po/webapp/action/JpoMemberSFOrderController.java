 ...
 *
 */
public class JpoMemberSFOrderController extends BaseController implements Controller {
    private final Log log = LogFactory.getLog(JpoMemberSFOrderController.class);
    ...
    throws Exception {
        ...
        if("C".equals(loginSysUser.getUserType())){
            RequestUtil.freshSession(request,"poMemberSFOrders",3l);
        ...
        if(!StringUtils.isEmpty(crm.getString("search","")) || !StringUtils.isEmpty(crm.getString("sysUser.userCode",""))){
        	if("C".equals(loginSysUser.getUserType())){
        		if(RequestUtil.saveOperationSession(request,"poMemberSFOrders",3l)!=0){
        			return new ModelAndView("redirect:jpoMemberSFOrders.html");
        		}
        	}else if("M".equals(loginSysUser.getUserType())){
        		if(RequestUtil.saveOperationSession(request,"poMemberSFOrders",3l)!=0){
        			return new ModelAndView("redirect:jpoMemberSFOrders.html");
        ...
        if(RequestUtil.isMobileRequest(request)){
        	return new ModelAndView("mobile/po/jpoMemberSFOrderList", Constants.JPOMEMBERORDER_LIST, jpoMemberOrders);
        }
        return new ModelAndView("po/jpoMemberSFOrderList", Constants.JPOMEMBERORDER_LIST, jpoMemberOrders);
    }
}
