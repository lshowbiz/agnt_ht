 ...
 *
 */
public class JpoMemberSROrderLCFormController extends BaseFormController {/*
    ...
    public JpoMemberSROrderLCFormController() {
    ...
    throws Exception {
        ...
        if ("deletePoMemberSROrderLC".equals(strAction) || "editPoMemberSROrderLC".equals(strAction) && !StringUtils.isEmpty(moId)){
    	...
    	if(loginSysUser.getIsMember()){
    		jpmProductSales = jpmProductSaleManager.getLCProductSales(loginSysUser.getCompanyCode(),Constants.STORE_MAINTAIN_PURCHASE,"1");
    	}else{
    		jpmProductSales = jpmProductSaleManager.getLCProductSales(loginSysUser.getCompanyCode(),Constants.STORE_MAINTAIN_PURCHASE,"2");
    	}
    	request.setAttribute("jpmProductSales", jpmProductSales);
    	
		if (RequestUtil.isMobileRequest(request)) {
			this.setFormView("/mobile/po/jpoMemberSROrderLCForm");
		} else {
			this.setFormView("/po/jpoMemberSROrderLCForm");
    ...
    throws Exception {
		...
		String key=null;
		String strAction = request.getParameter("strAction");
		if ("deletePoMemberSROrderLC".equals(strAction)) {
			...
			//if("addPoMemberSROrderLC".equals(strAction)){
				CommonRecord crm = new CommonRecord();
				...
				if(jpoMemberOrders.size()==0){
            		errors.reject("poMemberSFOrder.isNotExist", new Object[] {},LocaleUtil.getLocalText(loginSysUser.getDefCharacterCoding(),"poMemberSFOrder.isNotExist"));
            		return showForm(request, response, errors);
				}
				if(jmiMember==null){
            		errors.reject("miMember.notFound", new Object[] {},LocaleUtil.getLocalText(loginSysUser.getDefCharacterCoding(),"miMember.notFound"));
    ...
    *//**
    ...
    private boolean checkAmount(JpoMemberOrder jpoMemberOrder){
    	BigDecimal amount =new BigDecimal(ConfigUtil.getConfigValue(jpoMemberOrder.getSysUser().getCompanyCode().toUpperCase(), "store.r.order.amount"));
    ...
    private int calcOrderAmount(HttpServletRequest request, Set jpoMemberOrderSet, Set jpoMemberOrderFeeSet, JpoMemberOrder jpoMemberOrder){
    	...
    	while (its1.hasNext()) {
			...
			if(!"LC".equals(jpoMemberOrderList.getProductType())){
				...
				sumPrice1 = sumPrice1.add(jpoMemberOrderList.getJpmProductSale().getStoreMpPrice().multiply(new BigDecimal(jpoMemberOrderList.getQty())));
				sumPV1 = sumPV1.add(jpoMemberOrderList.getJpmProductSale().getStoreMpPv().multiply(new BigDecimal(jpoMemberOrderList.getQty()))) ;
			}
		}
    	...
    	if("CN".equals(jpoMemberOrder.getCompanyCode())){
        	if(sumPrice.compareTo(new BigDecimal("3000"))!=-1){
        		...
        		if((curDate.after(startDate))&&(curDate.before(endDate))){
        	    	...
        	    	while (its1.hasNext()) {
        				JpoMemberOrderList jpoMemberOrderList = (JpoMemberOrderList) its1.next();
        				if(!"LC".equals(jpoMemberOrderList.getProductType()) && !"NLC000000000CN0".equals(jpoMemberOrderList.getJpmProductSale().getJpmProduct().getProductNo())){
    ...
    *//**
    ...
    private JpoMemberOrderFee shippingFeeCalc(JpoMemberOrder jpoMemberOrder, Set jpoMemberOrderSet, HttpServletRequest request){
    	if("CN".equals(jpoMemberOrder.getCompanyCode())){
        	...
        	while (its1.hasNext()) {
        		JpoMemberOrderList jpoMemberOrderList = (JpoMemberOrderList) its1.next();
        		sumWeight = sumWeight.add(jpoMemberOrderList.getJpmProductSale().getWeight().multiply(new BigDecimal(jpoMemberOrderList.getQty())));
        		sumVolume = sumVolume.add(jpoMemberOrderList.getJpmProductSale().getVolume().multiply(new BigDecimal(jpoMemberOrderList.getQty())));
	    		...
	    		if(list.get(jpoMemberOrderList.getJpmProductSale().getJpmProduct().getProductNo())!=null){
	    			dtProduct = 1;
	    		}
        	}
    		CommonRecord crm = new CommonRecord();
    ...
    private Set fillInJfoMemberOrderList(HttpServletRequest request, JpoMemberOrder jpoMemberOrder){
		...
		for(int i_no = 0 ; i_no < g_no.length ; i_no++){
			if( !StringUtils.isEmpty(qty[i_no]) && !"0".equals(qty[i_no])){
				...
				if("2".equals(step)){
					jpoMemberOrderList.setPrice(jpmProductSale.getStoreMpPrice());
					jpoMemberOrderList.setPv(jpmProductSale.getStoreMpPv());
