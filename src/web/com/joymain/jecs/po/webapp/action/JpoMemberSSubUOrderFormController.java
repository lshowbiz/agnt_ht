 ...
 *
 */
public class JpoMemberSSubUOrderFormController extends BaseFormController {
    ...
    public JpoMemberSSubUOrderFormController() {
    ...
    throws Exception {
        ...
        if ("deletePoMemberSSubUOrder".equals(strAction) || "editPoMemberSSubUOrder".equals(strAction) && !StringUtils.isEmpty(moId)){
...
//        String teamCode=jmiMemberTeamManager.teamStr(loginSysUser);//获取团队编号
    	...
    	if(loginSysUser.getIsMember()){
    		if("2".equals(loginSysUser.getJmiMember().getIsstore())){
    			jpmProductSales = jpmProductSaleManager.getPmProductSalesNew(
    					...
    					Constants.SUBSTORE_UPDATE_PURCHASE,"1",teamCode,loginSysUser);
    		}
    	}else{
    		jpmProductSales = jpmProductSaleManager.getPmProductSalesNew(
    				loginSysUser.getCompanyCode(),
    				Constants.SUBSTORE_UPDATE_PURCHASE,"2",teamCode,loginSysUser);
    	}
    	request.setAttribute("jpmProductSales", jpmProductSales);
    	
    	if (RequestUtil.isMobileRequest(request)) {
			this.setFormView("/mobile/po/jpoMemberSSubUOrderForm");
		} else {
			this.setFormView("/po/jpoMemberSSubUOrderForm");
    ...
    throws Exception {
    	...
    	SysUser operatorSysUser = SessionLogin.getOperatorUser(request);
     	if("M".equals(loginSysUser.getUserType())){
        	  if(RequestUtil.saveOperationSession(request,"addPoMemberSSubUOrder", 10l)!=0){
        		  return new ModelAndView(getSuccessView());
        		 
        	  }
        }else if("C".equals(loginSysUser.getUserType()))
        {
            if(RequestUtil.saveOperationSession(request, "addPoMemberSSubUOrder", 30l)!=0){
        			  return new ModelAndView(getSuccessView());
             }
        }
		...
		String key=null;
		String strAction = request.getParameter("strAction");
		if ("deletePoMemberSSubUOrder".equals(strAction)) {
			jpoMemberOrderManager.removeJpoMemberOrder(jpoMemberOrder.getMoId().toString());
			key="poMemberOrder.deleted";
		}else{
			
			if("addPoMemberSSubUOrder".equals(strAction)){//判断升级单是否存在，只能升级一次
				CommonRecord crm = new CommonRecord();
				...
				if(jpoMemberOrders.size()!=0){
            		...
            		RequestUtil.cleanOperationSession(request,"addPoMemberSSubUOrder");
			...
			if("C".equals(loginSysUser.getUserType())){//判断会员是否存在，公司于会员是否为同一国别
				if(jmiMember==null){
...
//			if("0".equals(jmiMember.getCardType())){//待审会员不允许下店铺升级单
...
//			}
    		...
    		try{
        		...
        		if(result==1){
            		errors.reject("pv.notEnoughs", new Object[] {"N02330200101CN0"},LocaleUtil.getLocalText(loginSysUser.getDefCharacterCoding(),"pv.notEnoughs"));
            		RequestUtil.cleanOperationSession(request,"addPoMemberSSubUOrder");
            		return showForm(request, response, errors);
        		}
        ...
        /*			java.util.Calendar startc=java.util.Calendar.getInstance();
    			...
    			if("addPoMemberSSubUOrder".equals(strAction)){
    ...
    private boolean checkAmount(JpoMemberOrder jpoMemberOrder){
    	BigDecimal pv =new BigDecimal(ConfigUtil.getConfigValue(jpoMemberOrder.getSysUser().getCompanyCode().toUpperCase(), "store.u2.order.pv"));//0pv
    	if(pv.compareTo(jpoMemberOrder.getPvAmt())==1){
    		return false;
    	}
    	BigDecimal amount =new BigDecimal(ConfigUtil.getConfigValue(jpoMemberOrder.getSysUser().getCompanyCode().toUpperCase(), "store.u2.order.amount"));//165000
    ...
    private int calcOrderAmount(HttpServletRequest request, Set jpoMemberOrderSet, Set jpoMemberOrderFeeSet, JpoMemberOrder jpoMemberOrder){
    	...
    	while (its1.hasNext()) {
    		...
    		if("N02330200101CN0".equals(jpoMemberOrderList.getJpmProductSaleTeamType().getJpmProductSaleNew().getJpmProduct().getProductNo())){// 	二级晋升一级保证金+生活馆展示产品
    			isPass = true;
    		}
		}
...
/*        		if("P04010118001CN0".equals(jpoMemberOrderList.getJpmProductSale().getJpmProduct().getProductNo())){
	    		...
	    		if("N02330200101CN0".equals(jpoMemberOrderList.getJpmProductSale().getJpmProduct().getProductNo())){
	    			dtProduct = 1;
	    		}
    ...
    private Set fillInJfoMemberOrderList(HttpServletRequest request, JpoMemberOrder jpoMemberOrder){
		...
		for(int i_no = 0 ; i_no < g_no.length ; i_no++){
			if( !StringUtils.isEmpty(qty[i_no]) && !"0".equals(qty[i_no])){
    			...
    			if("N02330200101CN0".equals(jpmProductSale.getJpmProductSaleNew().getJpmProduct().getProductNo())){// 	二级晋升一级保证金+生活馆展示产品
