 ...
 *
 */
public class JpoMemberSSubUOrderController extends BaseController implements Controller {
    private final Log log = LogFactory.getLog(JpoMemberSSubUOrderController.class);
    ...
    throws Exception {
        ...
        if("C".equals(loginSysUser.getUserType())){
            RequestUtil.freshSession(request,"poMemberSSubUOrders",3l);
        ...
        if(!StringUtils.isEmpty(crm.getString("search","")) || !StringUtils.isEmpty(crm.getString("sysUser.userCode",""))){
        	if("C".equals(loginSysUser.getUserType())){
        		if(RequestUtil.saveOperationSession(request,"jpoMemberSSubUOrders",3l)!=0){
        			return new ModelAndView("redirect:jpoMemberSSubUOrders.html");
        		}
        	}else if("M".equals(loginSysUser.getUserType())){
        		if(RequestUtil.saveOperationSession(request,"jpoMemberSSubUOrders",3l)!=0){
        			return new ModelAndView("redirect:jpoMemberSSubUOrders.html");
        ...
        if (RequestUtil.isMobileRequest(request)) {
			return new ModelAndView("mobile/po/jpoMemberSSubUOrderList", Constants.JPOMEMBERORDER_LIST, jpoMemberOrders);
		}
        return new ModelAndView("po/jpoMemberSSubUOrderList", Constants.JPOMEMBERORDER_LIST, jpoMemberOrders);
    }
}
