 ...
 *
 */
public class JpoMemberRSOrderLCFormController extends BaseFormController {/*
    ...
    public JpoMemberRSOrderLCFormController() {
    ...
    throws Exception {
        ...
        if ("deletePoMemberRSOrderLC".equals(strAction) || "editPoMemberRSOrderLC".equals(strAction) && !StringUtils.isEmpty(moId)){
    	...
    	if(loginSysUser.getIsMember()){
    		jpmProductSales = jpmProductSaleManager.getLCProductSales(loginSysUser.getCompanyCode(),Constants.MAINTAIN_PURCHASE,"1");
    	}else{
    		jpmProductSales = jpmProductSaleManager.getLCProductSales(loginSysUser.getCompanyCode(),Constants.MAINTAIN_PURCHASE,"2");
    	}
    	request.setAttribute("jpmProductSales", jpmProductSales);
        
    	if (RequestUtil.isMobileRequest(request)) {
			this.setFormView("/mobile/po/jpoMemberRSOrderLCForm");
		} else {
			this.setFormView("/po/jpoMemberRSOrderLCForm");
		...
		if ("deletePoMemberRSOrderLC".equals(strAction)) {
    		...
    		try{
        		...
        		if(jpoMemberOrder.getSysUser().getJmiMember().getFreezeStatus()==0){
        			...
        			if(jpoMemberOrder.getPvAmt().compareTo(new BigDecimal("126"))==-1){
    ...
    private int calcOrderAmount(HttpServletRequest request, Set jpoMemberOrderSet, Set jpoMemberOrderFeeSet, JpoMemberOrder jpoMemberOrder){
    	...
    	while (its1.hasNext()) {
			...
			if(!"LC".equals(jpoMemberOrderList.getProductType())){
				...
				sumPrice1 = sumPrice1.add(jpoMemberOrderList.getJpmProductSale().getMpPrice().multiply(new BigDecimal(jpoMemberOrderList.getQty())));
				sumPV1 = sumPV1.add(jpoMemberOrderList.getJpmProductSale().getMpPv().multiply(new BigDecimal(jpoMemberOrderList.getQty())));
			}
		}
    	...
    	if(sumPV.compareTo(new BigDecimal("0"))==-1){
    		return 3;//小于600
    	}
    ...
    private Set fillInJfoMemberOrderList(HttpServletRequest request, JpoMemberOrder jpoMemberOrder){
		...
		for(int i_no = 0 ; i_no < g_no.length ; i_no++){
			if( !StringUtils.isEmpty(qty[i_no]) && !"0".equals(qty[i_no])){
				...
				if("2".equals(step)){
					jpoMemberOrderList.setPrice(jpmProductSale.getMpPrice());
					jpoMemberOrderList.setPv(jpmProductSale.getMpPv());
