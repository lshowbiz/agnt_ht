 ...
 *
 */
public class JpoMemberUOrderLCFormController extends BaseFormController {/*
    ...
    public JpoMemberUOrderLCFormController() {
    ...
    throws Exception {
        ...
        if ("deletePoMemberUOrderLC".equals(strAction) || "editPoMemberUOrderLC".equals(strAction) && !StringUtils.isEmpty(moId)){
    	...
    	if(loginSysUser.getIsMember()){
    		jpmProductSales = jpmProductSaleManager.getLCProductSales(loginSysUser.getCompanyCode(),Constants.UPDATE_PURCHASE,"1");
    	}else{
    		jpmProductSales = jpmProductSaleManager.getLCProductSales(loginSysUser.getCompanyCode(),Constants.UPDATE_PURCHASE,"2");
    	}
    	request.setAttribute("jpmProductSales", jpmProductSales);
    	
    	if(RequestUtil.isMobileRequest(request)){
        	this.setFormView("/mobile/po/jpoMemberUOrderLCForm");
        } else {
        	this.setFormView("/po/jpoMemberUOrderLCForm");
    ...
    *//**
    ...
    private boolean checkAmount(JpoMemberOrder jpoMemberOrder){
    	...
    	if(!"6".equals(jmiMember.getCardType())){
    		...
    		BigDecimal pv2;
    		if(cardType==5){
    			pv2=new BigDecimal(ConfigUtil.getConfigValue(jpoMemberOrder.getSysUser().getCompanyCode().toUpperCase(), "cardtype.1.value"));
    		}else{
    		...
    		if(pv2.subtract(pv1).compareTo(orderPv)!=1){
        		return true;
        	}
    	}
    	return false;
