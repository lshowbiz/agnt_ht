 ...
 *
 */
public class JpoMemberSROrderFormController extends BaseFormController {
    ...
    public JpoMemberSROrderFormController() {
    ...
    throws Exception {
        ...
        if ("deletePoMemberSROrder".equals(strAction) || "editPoMemberSROrder".equals(strAction) && !StringUtils.isEmpty(moId)){
...
//        String teamCode=jmiMemberTeamManager.teamStr(loginSysUser);//获取团队编号
    	...
    	if(loginSysUser.getIsMember()){
    		jpmProductSales = jpmProductSaleManager.getPmProductSalesNew(
    				...
    				Constants.STORE_MAINTAIN_PURCHASE,"1",teamCode,loginSysUser);
    	}else{
    		jpmProductSales = jpmProductSaleManager.getPmProductSalesNew(
    				loginSysUser.getCompanyCode(),
    				Constants.STORE_MAINTAIN_PURCHASE,"2",teamCode,loginSysUser);
    	}
      	
    	request.setAttribute("jpmProductSales", jpmProductSales);
  
		if (RequestUtil.isMobileRequest(request)) {
			this.setFormView("/mobile/po/jpoMemberSROrderForm");
		} else {
			this.setFormView("/po/jpoMemberSROrderForm");
    ...
    throws Exception {
    	...
    	SysUser operatorSysUser = SessionLogin.getOperatorUser(request);
    	if("M".equals(loginSysUser.getUserType())){
      	  if(RequestUtil.saveOperationSession(request,"addPoMemberSROrder", 10l)!=0){
      		  return new ModelAndView(getSuccessView());
      		 
      	  }
      }else if("C".equals(loginSysUser.getUserType()))
      {
          if(RequestUtil.saveOperationSession(request, "addPoMemberSROrder", 30l)!=0){
      			  return new ModelAndView(getSuccessView());
           }
      }
		...
		if ("deletePoMemberSROrder".equals(strAction)) {
    		...
    		try{
    			...
    			if("addPoMemberSROrder".equals(strAction)){
	    	...
	    	if((curDate.after(startDate))&&(curDate.before(endDate))){
	    		...
	    		while (its1.hasNext()) {
	    			...
	    			if(jpoMemberOrderList.getPv().compareTo(new BigDecimal("0"))==1){
	    				jpoMemberOrderList.setPrice(jpoMemberOrderList.getJpmProductSale().getStoreMpPrice().multiply(new BigDecimal("0.95")));
	    			}
	    		}
	    	}
		}*/
