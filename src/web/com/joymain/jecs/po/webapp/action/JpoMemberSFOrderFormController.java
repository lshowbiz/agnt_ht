 ...
 *
 */
public class JpoMemberSFOrderFormController extends BaseFormController {
	...
	public SysUserRoleManager getSysUserRoleManager() {
		return sysUserRoleManager;
	}
    ...
    public JpoMemberSFOrderFormController() {
    ...
    throws Exception {
        ...
        if ("deletePoMemberSFOrder".equals(strAction) || "editPoMemberSFOrder".equals(strAction) && !StringUtils.isEmpty(moId)){
...
//        String teamCode=jmiMemberTeamManager.teamStr(loginSysUser);//获取团队编号
    	...
    	if(loginSysUser.getIsMember()){
    		jpmProductSales = jpmProductSaleManager.getPmProductSalesNew(
    				...
    				Constants.STORE_FIRST_PURCHASE,"1",teamCode,loginSysUser);
    	}else{
    		jpmProductSales = jpmProductSaleManager.getPmProductSalesNew(
    				loginSysUser.getCompanyCode(),
    				Constants.STORE_FIRST_PURCHASE,"2",teamCode,loginSysUser);
    	}
    	request.setAttribute("jpmProductSales", jpmProductSales);
        
    	if(RequestUtil.isMobileRequest(request)){
        	this.setFormView("/mobile/po/jpoMemberSFOrderForm");
        } else {
        	this.setFormView("/po/jpoMemberSFOrderForm");
    ...
    throws Exception {
    	...
    	SysUser operatorSysUser = SessionLogin.getOperatorUser(request);
    	if("M".equals(loginSysUser.getUserType())){
        	  if(RequestUtil.saveOperationSession(request,"addPoMemberSFOrder", 10l)!=0){
        		  return new ModelAndView(getSuccessView());
        		 
        	  }
        }else if("C".equals(loginSysUser.getUserType()))
        {
            if(RequestUtil.saveOperationSession(request, "addPoMemberSFOrder", 30l)!=0){
        			  return new ModelAndView(getSuccessView());
             }
        }
		...
		String key=null;
		String strAction = request.getParameter("strAction");
		if ("deletePoMemberSFOrder".equals(strAction)) {
			...
			SysUserRole userRole = sysUserRoleManager.getSysUserRoleByUserCode(userCode);
			SysRole role = sysRoleManager.getSysRole(userRole.getRoleId().toString());
			
			if("addPoMemberSFOrder".equals(strAction)){
				
				if(validateOrder(jpoMemberOrder, errors, loginSysUser)){
		    		return showForm(request, response, errors);
		    	}
				
				if(role.getRoleCode().equalsIgnoreCase(Constants.JOCS_STORE1_X) || 
						role.getRoleCode().equalsIgnoreCase(Constants.JOCS_STORE21)){
					
				} else {
					errors.reject("userRole.error", new Object[] {},
							LocaleUtil.getLocalText(
									loginSysUser.getDefCharacterCoding(),
									"userRole.error"));
					RequestUtil.cleanOperationSession(request,
							"addPoMemberSFOrder");
					return showForm(request, response, errors);
				}
			...
			if(jmiStore == null){
					//未申请
					errors.reject("poMemberSFOrder.errorRole", new Object[] {},LocaleUtil.getLocalText(loginSysUser.getDefCharacterCoding(),"poMemberSFOrder.errorRole"));
            		RequestUtil.cleanOperationSession(request,"addPoMemberSFOrder");
            		return showForm(request, response, errors);
			}else{
					
					if(jmiStore.getCheckStatus().equals(0)){
						
					}else if(jmiStore.getCheckStatus().equals(1)){
						//已是正式一级店
						errors.reject("poMemberSFOrder.isExist", new Object[] {},LocaleUtil.getLocalText(loginSysUser.getDefCharacterCoding(),"poMemberSFOrder.isExist"));
	            		RequestUtil.cleanOperationSession(request,"addPoMemberSFOrder");
	            		return showForm(request, response, errors);
						
					}else if(jmiStore.getCheckStatus().equals(3)){
						//申请失败
						errors.reject("poMemberSFOrder.checkFaild", new Object[] {},LocaleUtil.getLocalText(loginSysUser.getDefCharacterCoding(),"poMemberSFOrder.checkFaild"));
	            		RequestUtil.cleanOperationSession(request,"addPoMemberSFOrder");
	            		return showForm(request, response, errors);
			...
			if("C".equals(loginSysUser.getUserType())){//判断会员是否存在，公司与会员国别是否一致
				if(jmiMember==null){
			...
			if(this.checkPoMemberOrder(jpoMemberOrder, errors)){//店铺的验证
				RequestUtil.cleanOperationSession(request,"addPoMemberSFOrder");
				return showForm(request, response, errors);
			}
			
			if("1".equals(jpoMemberOrder.getPickup())){//会员自动取货
    		...
    		try{
        		...
        		if(result==1){
            		errors.reject("pv.notEnoughs", new Object[] {"N01200200101CN0"},LocaleUtil.getLocalText(loginSysUser.getDefCharacterCoding(),"pv.notEnoughs"));
            		RequestUtil.cleanOperationSession(request,"addPoMemberSFOrder");
            		return showForm(request, response, errors);
        		}
        		boolean enoughtPV = this.checkAmount(jpoMemberOrder);//判断订单总金额是否已经达到25000
        		if(enoughtPV==false){
    ...
    private boolean checkAmount(JpoMemberOrder jpoMemberOrder){
    	BigDecimal amount  =new BigDecimal(ConfigUtil.getConfigValue(jpoMemberOrder.getSysUser().getCompanyCode().toUpperCase(), "store.f.order.amount"));//25000
    ...
    private int calcOrderAmount(HttpServletRequest request, Set jpoMemberOrderSet, Set jpoMemberOrderFeeSet, JpoMemberOrder jpoMemberOrder){
    	...
    	while (its1.hasNext()) {
    		...
    		if("N01200200101CN0".equals(jpoMemberOrderList.getJpmProductSaleTeamType().getJpmProductSaleNew().getJpmProduct().getProductNo())){// 	一级生活馆辅销品+保证金+生活馆展示产品
    			isPass = true;
    		}
		}
    	...
    	Iterator its2 = jpoMemberOrderFeeSet.iterator();//算物流费
    	while (its2.hasNext()) {
    ...
    private Set fillInJfoMemberOrderFee(HttpServletRequest request, JpoMemberOrder jpoMemberOrder,Set jpoMemberOrderSet){
    	...
    	if("CN".equals(jpoMemberOrder.getCompanyCode()) || "HK".equals(jpoMemberOrder.getCompanyCode()) ){
        	...
        	if(jpoMemberOrderFee!=null){
        		...
        		jpoMemberOrderFee.setFeeType("1");//手续费
    ...
    private Set fillInJfoMemberOrderList(HttpServletRequest request, JpoMemberOrder jpoMemberOrder){
		...
		for(int i_no = 0 ; i_no < g_no.length ; i_no++){
			if( !StringUtils.isEmpty(qty[i_no]) && !"0".equals(qty[i_no])){
				...
				if("N01200200101CN0".equals(jpmProductSale.getJpmProductSaleNew().getJpmProduct().getProductNo())){// 	一级生活馆辅销品+保证金+生活馆展示产品
	    	...
	    	if((curDate.after(startDate))&&(curDate.before(endDate))){
	    		...
	    		while (its1.hasNext()) {
	    			...
	    			if(jpoMemberOrderList.getPv().compareTo(new BigDecimal("0"))==1){
	    				jpoMemberOrderList.setPrice(jpoMemberOrderList.getJpmProductSale().getStoreFpPrice().multiply(new BigDecimal("0.95")));
	    			}
	    		}
	    	}
		}*/
