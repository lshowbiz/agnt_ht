 ...
 *
 */
public class JpoMemberSFOrderLCFormController extends BaseFormController {/*
    ...
    public JpoMemberSFOrderLCFormController() {
    ...
    throws Exception {
        ...
        if ("deletePoMemberSFOrderLC".equals(strAction) || "editPoMemberSFOrderLC".equals(strAction) && !StringUtils.isEmpty(moId)){
    	...
    	if(loginSysUser.getIsMember()){
    		jpmProductSales = jpmProductSaleManager.getLCProductSales(loginSysUser.getCompanyCode(),Constants.STORE_FIRST_PURCHASE,"1");
    	}else{
    		jpmProductSales = jpmProductSaleManager.getLCProductSales(loginSysUser.getCompanyCode(),Constants.STORE_FIRST_PURCHASE,"2");
    	}
       ...
       // request.setAttribute("fixeds", sumPrice2.add(sumPrice3));
    	...
    	request.setAttribute("pvamt", ConfigUtil.getConfigValue(loginSysUser.getCompanyCode().toUpperCase(), "store.f.order.pvamt"));
        
    	if (RequestUtil.isMobileRequest(request)) {
         	this.setFormView("/mobile/po/jpoMemberSFOrderLCForm");
        } else {
        	this.setFormView("/po/jpoMemberSFOrderLCForm");
    ...
    *//**
    ...
    private boolean checkAmount(JpoMemberOrder jpoMemberOrder){
    	BigDecimal pvAmt = new BigDecimal(ConfigUtil.getConfigValue(jpoMemberOrder.getSysUser().getCompanyCode().toUpperCase(), "store.f.order.pvamt"));
