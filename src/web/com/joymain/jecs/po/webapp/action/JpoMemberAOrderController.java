 ...
 *
 */
public class JpoMemberAOrderController extends BaseController implements Controller {
    private final Log log = LogFactory.getLog(JpoMemberAOrderController.class);
    ...
    throws Exception {
        ...
        if("C".equals(loginSysUser.getUserType())){
            RequestUtil.freshSession(request,"poMemberAOrders",3l);
        ...
        if(!StringUtils.isEmpty(crm.getString("search","")) || !StringUtils.isEmpty(crm.getString("sysUser.userCode",""))){
        	if("C".equals(loginSysUser.getUserType())){
        		if(RequestUtil.saveOperationSession(request,"poMemberAOrders",3l)!=0){
        			return new ModelAndView("redirect:jpoMemberAOrders.html");
        		}
        	}else if("M".equals(loginSysUser.getUserType())){
        		if(RequestUtil.saveOperationSession(request,"poMemberAOrders",3l)!=0){
        			return new ModelAndView("redirect:jpoMemberAOrders.html");
        ...
        if (RequestUtil.isMobileRequest(request)) {
			return new ModelAndView("mobile/po/jpoMemberAOrderList", Constants.JPOMEMBERORDER_LIST, jpoMemberOrders);
		}
        return new ModelAndView("po/jpoMemberAOrderList", Constants.JPOMEMBERORDER_LIST, jpoMemberOrders);
    }
}
