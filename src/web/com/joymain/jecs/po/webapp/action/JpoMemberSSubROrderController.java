 ...
 *
 */
public class JpoMemberSSubROrderController extends BaseController implements Controller {
    private final Log log = LogFactory.getLog(JpoMemberSSubROrderController.class);
    ...
    throws Exception {
        ...
        if("C".equals(loginSysUser.getUserType())){
            RequestUtil.freshSession(request,"poMemberSSubROrders",3l);
        ...
        if(!StringUtils.isEmpty(crm.getString("search","")) || !StringUtils.isEmpty(crm.getString("sysUser.userCode",""))){
        	if("C".equals(loginSysUser.getUserType())){
        		if(RequestUtil.saveOperationSession(request,"jpoMemberSSubROrders",3l)!=0){
        			return new ModelAndView("redirect:jpoMemberSSubROrders.html");
        		}
        	}else if("M".equals(loginSysUser.getUserType())){
        		if(RequestUtil.saveOperationSession(request,"jpoMemberSSubROrders",3l)!=0){
        			return new ModelAndView("redirect:jpoMemberSSubROrders.html");
        ...
        if (RequestUtil.isMobileRequest(request)) {
			return new ModelAndView("mobile/po/jpoMemberSSubROrderList", Constants.JPOMEMBERORDER_LIST, jpoMemberOrders);
		}

        return new ModelAndView("po/jpoMemberSSubROrderList", Constants.JPOMEMBERORDER_LIST, jpoMemberOrders);
    }
}
