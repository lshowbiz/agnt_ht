        ...
        if ("importPoMemberOrder".equals(request.getParameter("strAction"))) {
        	...
        	try {
				...
				for (int i = 1; i < sheet0.getRows(); i++) {
					//会员编号,会员名称,收货人FirstName,收货人LastName,联系电话,收货市,收货地址,邮编,仓库号,备注
...
/*					LinkedHashMap list = ListUtil.getListOptions(SessionLogin.getLoginUser(request).getCompanyCode(), "feetype");
					...
					String teamCode = eu.getContents(sheet0, 12, i);//用于保存团队顶点编号
					...
					List list = jpoMemberOrderManager.getJpoMemberOrdersByCrm(crm, new Pager(request, 0));
					if(list.size()!=0){
						throw new Exception(i + getText("poMemberFOrder.isExist"));
					}
					...
					for (int m = 1; m < sheet1.getRows(); m++) {
						...
						JpmProductSaleTeamType jpmProductSale = jpmProductSaleManager.getJpmProductSaleTeamType(companyCode, productNo,jpoMemberOrder.getOrderType(),jpoMemberOrder.getTeamCode());
						...
						amount = amount.add(new BigDecimal(price).multiply(new BigDecimal(qty)));
						jpoMemberOrderListSet.add(jpoMemberOrderList);
					}
					jpoMemberOrder.setPvAmt(pvAmt);
					if(pvAmt.compareTo(new BigDecimal("70"))==-1){
