...
public class JmiSubStoreController extends BaseController implements Controller {
    private final Log log = LogFactory.getLog(JmiSubStoreController.class);
...
//        }
    	...
    	if("businessLiceseJmiSubStore".equals(strAction)||"contracJmiSubStore".equals(strAction)||"storePicJmiSubStore".equals(strAction)||"noticeTimeJmiSubStore".equals(strAction)){
        ...
        return new ModelAndView("mi/jmiSubStoreList", Constants.JMISUBSTORE_LIST, jmiSubStores);
    }
	...
	private void getReport(List jmiSubStores,HttpServletRequest request, HttpServletResponse response,SysUser defSysUser) throws Exception{
		...
		eu.addString(wsheet, i++, index, LocaleUtil.getLocalText("subStore.address1"));
...
//		eu.addString(wsheet, i++, index, LocaleUtil.getLocalText("busi.user.check"));
//		eu.addString(wsheet, i++, index, LocaleUtil.getLocalText("jmiSubStore.checkRemark"));
//		eu.addString(wsheet, i++, index, LocaleUtil.getLocalText("pd.confirmUserNo"));
...
//		eu.addString(wsheet, i++, index, LocaleUtil.getLocalText("jmiSubStore.noticeTime"));
		eu.addString(wsheet, i++, index, LocaleUtil.getLocalText("pd.okTime"));
...
//			eu.addString(wsheet, i++, index,jmiSubStore.getMobiletele());
			...
			eu.addString(wsheet, i++, index,alStateProvinceStr+alCityStr+alDistrictStr+formatString(jmiSubStore.getAddress()));
			...
			if(!StringUtil.isEmpty(jmiSubStore.getJmiMember().getSubRecommendStore()) ){
				JmiMember jmiMember=jmiMemberManager.getJmiMember(jmiSubStore.getJmiMember().getSubRecommendStore());
				if(jmiMember!=null){
					eu.addString(wsheet, i++, index,jmiMember.getMiUserName());
				}else{
...
//			eu.addString(wsheet, i++, index,LocaleUtil.getLocalText(checkStatusMap.get(jmiSubStore.getJmiMember().getSubStoreStatus()).toString()));
//			eu.addString(wsheet, i++, index,LocaleUtil.getLocalText(confirmStatusMap.get(jmiSubStore.getConfirmStatus()).toString()));
//			eu.addString(wsheet, i++, index,formatString(jmiSubStore.getCheckUser()));
//			eu.addString(wsheet, i++, index,formatString(jmiSubStore.getCheckRemark()));
//			eu.addString(wsheet, i++, index,formatString(jmiSubStore.getConfirmUser()));
...
//			}
			if(null!=jmiSubStore.getConfirmTime()){
				eu.addString(wsheet, i++, index,jmiSubStore.getConfirmTime().toString());
			}else{
				eu.addString(wsheet, i++, index, "");
			}
