 ...
 *
 */
public class FiIncExpStatController extends BaseController implements Controller {
	private final Log log = LogFactory.getLog(FiIncExpStatController.class);
	...
	public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		List fiBankbookJournals = this.fiBankbookJournalManager.getFiBankbookJournalsByCrm(crm, pager);
		
		//根据数据重新设置分页数据
		request.setAttribute("fiBankbookJournalListTable_totalRows", pager.getTotalObjects());

		Map incExpMap = this.fiBankbookJournalManager.getIncExpStatMap(crm);
		...
		return new ModelAndView("fi/fi_inc_exp_stat", Constants.FIBANKBOOKJOURNAL_LIST, fiBankbookJournals);
	}
}
