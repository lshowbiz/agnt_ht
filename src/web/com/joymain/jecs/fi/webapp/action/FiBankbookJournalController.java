...
public class FiBankbookJournalController extends BaseController implements Controller {
    private final Log log = LogFactory.getLog(FiBankbookJournalController.class);
    ...
    throws Exception {
        ...
        if("C".equals(loginSysUser.getUserType())){
            RequestUtil.freshSession(request,"listFiBankbookJournalsJJ",10l);
    	}else if("M".equals(loginSysUser.getUserType())){
            RequestUtil.freshSession(request,"listFiBankbookJournalsJJ",1l);
    	}
		...
		if (request.getParameter("search") != null || SessionLogin.getLoginUser(request).getIsMember() || SessionLogin.getLoginUser(request).getIsAgent()){
			...
			fiBankbookJournals = this.fiBankbookJournalManager.getFiBankbookJournalsByCrm(crm, pager);
			
			//Modify By WuCF 20140320 查询结果的总计 
			Map sumMap = this.fiBankbookJournalManager.getSumAmountByCrm(crm);
        ...
        return new ModelAndView("fi/fiBankbookJournalList", Constants.FIBANKBOOKJOURNAL_LIST, fiBankbookJournals);
    }
}
