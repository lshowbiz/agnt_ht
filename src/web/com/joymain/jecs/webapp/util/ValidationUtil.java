...
public class ValidationUtil {
	...
	public static boolean validateLengthRang(Object bean, ValidatorAction va, Field field, Errors errors) {
		String value = ValidatorUtils.getValueAsString(bean, field.getProperty());
		int min = new Integer(field.getVarValue("min")).intValue();
		int max = new Integer(field.getVarValue("max")).intValue();
		int length;
		if (!GenericValidator.isBlankOrNull(value)) {
			length = value.length();
