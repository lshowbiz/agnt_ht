...
<script>
function nextSave(){
	if(Number($('total_price').innerText)>=Number($('total_price_s').innerText)*2){
		if(isFormPosted()){
			return true;
		}else{
			return false;
...
<jecs:power powerCode="deletePoMemberSSubROrderLC">
				<input type="submit" class="button" name="delete" onclick="bCancel=true;return confirmDelete(this.form,'PoMemberSSubROrderLC')" value="<jecs:locale key="operation.button.delete"/>" />
</jecs:power>
</c:if>
...
<script>
  ...
  function amt_count(step){
	if(step==2){
    	...
    	$('total_price_pre').innerHTML=Math.round(nPriceAmt/2,2);
	}else{
		var nPriceAmt=Number(0);
		var nPvAmt=Number(0);
		var nPvQty=Number(0);
...
<input type="button" class="button" name="back"  onclick="window.location.href='jpoMemberSSubROrders.html?needReload=1'" value="<jecs:locale key="operation.button.return"/>" />
