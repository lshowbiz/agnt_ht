...
<form action="jpoMemberSSubUOrders.html" method="get" name="searchForm"
	id="searchForm">
	<div class="searchBar">
				...
				<c:if test='${canAdd == "1" }'>
					...
					<c:if test="${sessionScope.sessionLogin.userType=='C'}">
						<button name="add" class="btn btn_ins" type="button"
							onclick="location.href='<c:url value="/editJpoMemberSSubUOrder.html?strAction=addPoMemberSSubUOrder"/>'">
					...
					<c:if test="${sessionScope.sessionLogin.userType!='C'}">
						<c:if
							...
							<c:if test="${sessionScope.sessionLogin.companyCode=='CN' }">
								<button name="add" class="btn btn_ins" type="button"
									onclick="location.href='<c:url value="/editJpoMemberSSubUOrderLC.html?strAction=addPoMemberSSubUOrderLC"/>'">
									<jecs:locale key="member.new.num" />
								</button>
							</c:if>
							<c:if test="${sessionScope.sessionLogin.companyCode!='CN' }">
...
<ec:table tableId="jpoMemberOrderListTable" items="jpoMemberOrderList"
	...
	action="${pageContext.request.contextPath}/jpoMemberSSubUOrders.html"
			...
			<c:if
				...
				<c:if test="${jpoMemberOrder.productType!='LC' }">
					<jecs:power powerCode="editPoMemberSSubUOrder">
						<img src="<c:url value='/images/newIcons/pencil_16.gif'/>"
							onclick="javascript:editJpoMemberSSubUOrder('${jpoMemberOrder.moId}')"
				...
				<c:if test="${jpoMemberOrder.productType=='LC' }">
					<jecs:power powerCode="editPoMemberSSubUOrderLC">
						<img src="<c:url value='/images/newIcons/pencil_16.gif'/>"
							onclick="javascript:window.location='editJpoMemberSSubUOrderLC.html?strAction=editPoMemberSSubUOrderLC&moId=${jpoMemberOrder.moId}'"
							style="cursor: pointer;" title="<jecs:locale key="button.edit"/>" />
					</jecs:power>
				</c:if>
			</c:if>
...
<script type="text/javascript">
   ...
   function editJpoMemberSSubUOrder(moId){
   		<jecs:power powerCode="editPoMemberSSubUOrder">
					window.location="editJpoMemberSSubUOrder.html?strAction=editPoMemberSSubUOrder&moId="+moId;
			</jecs:power>
		}
