...
<jecs:power powerCode="deletePoMemberBOrder">
				<button style="font-size:14px;height:30px;" type="submit" class="btn btn_sele" name="delete" onclick="bCancel=true;return confirmDelete(this.form,'PoMemberBOrder')" >
					<jecs:locale key="operation.button.delete"/>
				</button>
</jecs:power>
</c:if>

<button style="font-size:14px;height:30px;" type="button" class="btn btn_sele" name="back"  onclick="window.location.href='jpoMemberBOrders.html?needReload=1'" >
...
function searchMiMember(){
	...
	window.location.href="editJpoMemberBOrder.html?strAction=addPoMemberBOrder&sysUser.userCode="+userCode;
}
</script>
<script>
  var aSub = Array();
...
<form:form commandName="jpoMemberOrder" method="post" action="editJpoMemberBOrder.html" onsubmit="return onSubmitCheck(this);" id="jpoMemberOrderForm">
...
<c:if test='${param.strAction=="editPoMemberBOrder"}'>
    <tr class="edit_tr">
    ...
    <td><span class="textbox">
    	...
    	<c:if test='${param.strAction=="addPoMemberBOrder"}'>
...
<c:if test="${empty jmiAddrBooks }">
    ...
    <td><span class="textbox">
       <form:input cssClass="textbox-text" path="mobiletele" id="mobiletele" />
    </span></td>
