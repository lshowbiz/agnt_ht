...
<jecs:power powerCode="deletePoMemberAOrder">
				<button style="font-size:14px;height:30px;" type="submit" class="btn btn_sele" name="delete" onclick="bCancel=true;return confirmDelete(this.form,'PoMemberAOrder')" >
					<jecs:locale key="operation.button.delete"/>
				</button>
</jecs:power>
</c:if>

<button style="font-size:14px;height:30px;" type="button" class="btn btn_sele" name="back"  onclick="window.location.href='jpoMemberAOrders.html?needReload=1'" >
...
function searchMiMember(){
	...
	window.location.href="editJpoMemberAOrder.html?strAction=addPoMemberAOrder&sysUser.userCode="+userCode;
}
</script>
<script>
  ...
  {	
     if(productNo=="P08420300101CN0" || productNo=="P01050100101CN0")
	  {
			if(nQty.value>0)
			{
    ...
    if (!isFinite(nQty.value) || nQty.value < 0 ){
	 ...
	 var nameQty=document.getElementsByName("qty");
		 if(productNo=="N06010100101CN0")
		 {		         
				 if(nQty.value>0)
				 {
							            for(j=0;j<nameQty.length;j++){
							           
							             if(nameQty[j].id!="qty"+no)
							             {
							               document.getElementById(nameQty[j].id).readOnly=true;
							               document.getElementById(nameQty[j].id).style.background='#DFE8F6';
							             }
							        }
							  ...
							  if(nQty.value>=200)
								 {
								  aSub[no]['price']='220'; 
								  $('changePrice'+no).innerHTML='220'
								 }else
								 {
								      aSub[no]['price']='300'; 
								      $('changePrice'+no).innerHTML='300'
								 }
				 }else
				 {
				           for(j=0;j<nameQty.length;j++){
							               document.getElementById(nameQty[j].id).readOnly=false; 
							               document.getElementById(nameQty[j].id).style.background='white';
							        }
		 ...
		 }else
		 {
		     var getValur=false;
		         ...
		         {
		            for(j=0;j<nameQty.length;j++){
		               var valueString=nameQty[j].id.substring(3,nameQty[j].id.length);
		               if(aSub[valueString]["productNo"]=="N06010100101CN0")
		               {
		                   document.getElementById(nameQty[j].id).readOnly=true;
						   document.getElementById(nameQty[j].id).style.background='#DFE8F6';
		               }
...
<form:form commandName="jpoMemberOrder" method="post" action="editJpoMemberAOrder.html" onsubmit="return onSubmitCheck(this);" id="jpoMemberOrderForm">
...
<c:if test='${param.strAction=="editPoMemberAOrder"}'>
    <tr class="edit_tr">
    ...
    <td><span class="textbox">
    	...
    	<c:if test='${param.strAction=="addPoMemberAOrder"}'>
...
<script>
	...
	<c:if test='${pol.jpmProductSaleTeamType.jpmProductSaleNew.jpmProduct.productNo=="N06010100101CN0" && pol.qty>=200}'>
	  $('changePrice'+${pol.jpmProductSaleTeamType.pttId}).innerHTML='220';
	    aSub[${pol.jpmProductSaleTeamType.pttId }]['price']='220'; 
	</c:if>
	</c:forEach>
	amt_count();
</script>
