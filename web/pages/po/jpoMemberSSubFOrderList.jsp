...
<form action="jpoMemberSSubFOrders.html" method="get" name="searchForm"
	id="searchForm">
	<div class="searchBar">
			...
			<jecs:power powerCode="addPoMemberSSubFOrder">
				...
				<c:if test="${sessionScope.sessionLogin.userType=='C'}">
					<button name="add" class="btn btn_ins" type="button"
						onclick="location.href='<c:url value="/editJpoMemberSSubFOrder.html?strAction=addPoMemberSSubFOrder"/>'">
				...
				<c:if test="${sessionScope.sessionLogin.userType!='C'}">
					<c:if
						...
						<c:if test="${sessionScope.sessionLogin.companyCode=='CN' }">
							<button name="add" class="btn btn_ins" type="button"
								onclick="location.href='<c:url value="/editJpoMemberSSubFOrderLC.html?strAction=addPoMemberSSubFOrderLC"/>'">
								<jecs:locale key="member.new.num" />
							</button>
						</c:if>
						<c:if test="${sessionScope.sessionLogin.companyCode!='CN' }">
...
<ec:table tableId="jpoMemberOrderListTable" items="jpoMemberOrderList"
	...
	action="${pageContext.request.contextPath}/jpoMemberSSubFOrders.html"
			...
			<c:if
				...
				<c:if test="${jpoMemberOrder.productType!='LC' }">
					<jecs:power powerCode="editPoMemberSSubFOrder">
						<img src="<c:url value='/images/newIcons/pencil_16.gif'/>"
							onclick="javascript:editJpoMemberSSubFOrder('${jpoMemberOrder.moId}')"
				...
				<c:if test="${jpoMemberOrder.productType=='LC' }">
					<jecs:power powerCode="editPoMemberSSubFOrderLC">
						<img src="<c:url value='/images/newIcons/pencil_16.gif'/>"
							onclick="javascript:window.location='editJpoMemberSSubFOrderLC.html?strAction=editPoMemberSSubFOrderLC&moId=${jpoMemberOrder.moId}'"
							style="cursor: pointer;" title="<jecs:locale key="button.edit"/>" />
					</jecs:power>
				</c:if>
			</c:if>
...
<script type="text/javascript">
	function editJpoMemberSSubFOrder(moId) {
		<jecs:power powerCode="editPoMemberSSubFOrder">
		window.location = "editJpoMemberSSubFOrder.html?strAction=editPoMemberSSubFOrder&moId="
				+ moId;
		</jecs:power>
	}
</script>
