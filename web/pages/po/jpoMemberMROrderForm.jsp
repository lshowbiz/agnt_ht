...
<jecs:power powerCode="deletePoMemberMROrder">
				<input type="submit" class="button" name="delete" onclick="bCancel=true;return confirmDelete(this.form,'PoMemberMROrder')" value="<jecs:locale key="operation.button.delete"/>" />
</jecs:power>
</c:if>

<input type="button" class="button" name="back"  onclick="window.location.href='jpoMemberMROrders.html?needReload=1'" value="<jecs:locale key="operation.button.return"/>" />
...
<form:form commandName="jpoMemberOrder" method="post" action="editJpoMemberMROrder.html" onsubmit="return onSubmitCheck(this);" id="jpoMemberOrderForm">
...
<c:if test='${param.strAction=="editPoMemberMROrder"}'>
    <tr><th>
    ...
    <td align="left">
    	...
    	<c:if test='${param.strAction=="addPoMemberMROrder"}'>
...
<c:if test="${sta.count!=1}">
     <div id="tabCot_product_${sta.count}" class="tabCot"  style="display: none;">
        ...
        <td>${item.dreamMpPrice}</td>
          <td>${item.dreamMpPv}</td>
          <td>
            <input type='text' class='text medium' name='qty' value='0' size='3' onchange="set_amt(this,'${item.uniNo }');" id=qty<c:out value="${item.uniNo }" />>
				    ...
				    <script>
						...
						aSub[<c:out value="${item.uniNo }" />]['price']='<c:out value="${item.dreamMpPrice }" />';
						aSub[<c:out value="${item.uniNo }" />]['pv']='<c:out value="${item.dreamMpPv }" />';
					</script>
          </td>
