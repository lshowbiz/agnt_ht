...
<form action="jpoMemberUOrders.html" method="get" name="searchForm" id="searchForm">
<div class="searchBar">
	...
	<div class="new_searchBar">
		<jecs:locale key="poMemberOrder.Times"/>
		<input id="startOrderTime" name="startOrderTime" type="text" value="${param.startOrderTime }" 
			style="cursor: pointer; width: 77px;" onclick="WdatePicker({dateFmt:'yyyy-MM-dd',readOnly:true})"/>
		-
		<input id="endOrderTime" name="endOrderTime" type="text" value="${param.endOrderTime }"
			style="cursor: pointer; width: 77px;" onclick="WdatePicker({dateFmt:'yyyy-MM-dd',readOnly:true})"/>
	</div>	
	<div class="new_searchBar">
	...
	<div class="new_searchBar" style="margin-left: 40px">
		...
		<c:if test="${sessionLogin.upGrade=='1' }">
			<c:if test="${sessionScope.sessionLogin.userType=='C'}">
				<button name="add" class="btn btn_ins" type="button" 
					onclick="location.href='<c:url value="/editJpoMemberUOrder.html?strAction=addPoMemberUOrder"/>'" >
...
<ec:table 
	...
	action="${pageContext.request.contextPath}/jpoMemberUOrders.html"
	width="100%"
...
<c:if test="${sessionScope.sessionLogin.userType=='C'}">
		...
		<c:if test="${jpoMemberOrder.productType!='LC' }">
   		<jecs:power powerCode="editPoMemberUOrder">
							<img src="<c:url value='/images/newIcons/pencil_16.gif'/>" 
								onclick="javascript:editJpoMemberUOrder('${jpoMemberOrder.moId}')"
		...
		<c:if test="${jpoMemberOrder.productType=='LC' }">
   		<jecs:power powerCode="editPoMemberUOrderLC">
							<img src="<c:url value='/images/newIcons/pencil_16.gif'/>" 
								onclick="javascript:window.location='editJpoMemberUOrderLC.html?strAction=editPoMemberUOrderLC&moId=${jpoMemberOrder.moId}'"
								style="cursor: pointer;" title="<jecs:locale key="button.edit"/>"/> 
		</jecs:power>
		</c:if>
					</c:if>
...
<script type="text/javascript">
   ...
   function editJpoMemberUOrder(moId){
   		<jecs:power powerCode="editPoMemberUOrder">
					window.location="editJpoMemberUOrder.html?strAction=editPoMemberUOrder&moId="+moId;
			</jecs:power>
		}
