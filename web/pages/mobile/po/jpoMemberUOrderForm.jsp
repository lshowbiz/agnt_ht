...
<c:if test='${empty jpoMemberOrder.moId}'>
	...
	onclick="window.location.href='editJpoMemberUOrderLC.html?strAction=addPoMemberUOrderLC'" value="兰超" />
</c:if>
</jecs:power>
...
<jecs:power powerCode="deletePoMemberFOrder">
	<input data-inline="true" type="submit" class="button" name="delete" onclick="bCancel=true;return confirmDelete(this.form,'PoMemberUOrder')" value="<jecs:locale key="operation.button.delete"/>" />
</jecs:power>
</c:if>
...
<c:if test="${sta.count!=1}">
     <div id="tabCot_product_${sta.count}" class="tabCot"  style="display: none;">
       ...
       <%--  <td>  
        <jecs:locale  key="busi.product.pic"/>
        </td>
        <td>  <jecs:locale  key="busi.product.productno"/>
        </td> --%>
        <td>  <jecs:locale  key="pmProduct.productName"/>
        </td>
        <td>  <jecs:locale  key="pd.price"/>
        </td>
        <td>  <jecs:locale  key="busi.direct.pv"/>
        </td>
