...
<jecs:power powerCode="addPoMemberSSubUOrder">
...
<c:if test='${empty jpoMemberOrder.moId}'>
	<input data-inline="true" type="button" class="button" name="addLC"  onclick="window.location.href='editJpoMemberSSubUOrderLC.html?strAction=addPoMemberSSubUOrderLC'" value="<jecs:locale key="operation.button.addLC"/>" />
</c:if>
</jecs:power>

<c:if test='${jpoMemberOrder.status=="1" && jpoMemberOrder.submitStatus =="1" && not empty jpoMemberOrder.moId}'>
<jecs:power powerCode="editPoMemberSSubUOrder">
				<input data-inline="true" type="submit" class="button" name="save"  onclick="bCancel=false" value="<jecs:locale key="operation.button.save"/>" />
</jecs:power>
<jecs:power powerCode="deletePoMemberSSubUOrder">
				<input data-inline="true" type="submit" class="button" name="delete" onclick="bCancel=true;return confirmDelete(this.form,'PoMemberSSubUOrder')" value="<jecs:locale key="operation.button.delete"/>" />
</jecs:power>
</c:if>
...
<form:form commandName="jpoMemberOrder" method="post" action="editJpoMemberSSubUOrder.html" onsubmit="return onSubmitCheck(this);" id="jpoMemberOrderForm">
...
<c:if test='${param.strAction=="editPoMemberSSubUOrder"}'>
    <tr><th>
    ...
    <td align="left">
    	...
    	<c:if test='${param.strAction=="addPoMemberSSubUOrder"}'>
...
<c:if test="${sta.count!=1}">
     <div id="tabCot_product_${sta.count}" class="tabCot"  style="display: none;">
        ...
        <td>${item.storeFpPrice}</td>
          <td>${item.storeFpPv}</td>
          <td align="left">
            <input type='text' class='text medium' name='qty' value='0' size='3' onchange="set_amt(this,'${item.uniNo }');" id=qty<c:out value="${item.uniNo }" />>
				    ...
				    <script>
						...
						aSub[<c:out value="${item.uniNo }" />]['price']='<c:out value="${item.storeFpPrice }" />';
						aSub[<c:out value="${item.uniNo }" />]['pv']='<c:out value="${item.storeFpPv }" />';
					</script>
          </td>
