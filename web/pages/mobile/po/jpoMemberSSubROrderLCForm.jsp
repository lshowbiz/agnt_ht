...
<jecs:power powerCode="editPoMemberSSubROrderLC">
				<input data-inline="true" type="submit" class="button" name="save"  onclick="return nextSave();" value="<jecs:locale key="operation.button.save"/>" />
</jecs:power>
<jecs:power powerCode="deletePoMemberSSubROrderLC">
				<input data-inline="true" type="submit" class="button" name="delete" onclick="bCancel=true;return confirmDelete(this.form,'PoMemberSSubROrderLC')" value="<jecs:locale key="operation.button.delete"/>" />
</jecs:power>
</c:if>
...
<form:form commandName="jpoMemberOrder" method="post" action="editJpoMemberSSubROrderLC.html" id="jpoMemberOrderForm">
...
<c:if test='${param.strAction=="editPoMemberSSubROrderLC"}'>
    <tr><th>
    ...
    <td align="left">
    	...
    	<c:if test='${param.strAction=="addPoMemberSSubROrderLC"}'>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td><br />
    	<jecs:code listCode="pmproduct.productcategory" value="${jpms.key }"/>,<jecs:label key="pd.price"/>${jpms.value.productInfo.substoreMpPrice },<jecs:label key="busi.direct.pv"/>${jpms.value.productInfo.substoreMpPv }<br />
		<br />
					...
					<c:forEach items="${jpms.value.size }" var="size">
						...
						<c:if test="${not empty jpms.value.jpmProductSales[style.PRODUCT_STYLE] && not empty jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE] }">
						    ...
						    <script>
								...
								aSub2[<c:out value="${jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE].uniNo }" />2]['price']='<c:out value="${jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE].substoreMpPrice }" />';
								aSub2[<c:out value="${jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE].uniNo }" />2]['pv']='<c:out value="${jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE].substoreMpPv }" />';
							</script><%--
							<jecs:label key="pd.price"/>${jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE].substoreMpPrice }<br/>
							<jecs:label key="busi.direct.pv"/>${jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE].substoreMpPv }
						--%></c:if>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td><br />
    	<jecs:code listCode="pmproduct.productcategory" value="${jpms.key }"/>,<jecs:label key="pd.price"/>${jpms.value.productInfo.substoreMpPrice },<jecs:label key="busi.direct.pv"/>0<br />
		<br />
		<table id="tablemain" class="tree_grid" border="0" cellspacing="0" cellpadding="0" width="40%">
			<thead>
				<tr>
					...
					<c:forEach items="${jpms.value.size }" var="size">
						...
						<c:if test="${not empty jpms.value.jpmProductSales[style.PRODUCT_STYLE] && not empty jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE] }">
						    ...
						    <script>
								...
								aSub3[<c:out value="${jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE].uniNo }" />3]['price']='<c:out value="${jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE].substoreMpPrice }" />';
