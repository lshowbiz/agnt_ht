...
window.open ('http://ec.joymaingroup.com/jecs/userfiles/Service_Contracts2.pdf');
</c:if>
</SCRIPT>
...
if(Number(${fixeds})==0)
{
	...
	if(Number($('total_price').innerText)-Number(document.getElementById("fixed").value)>=Number($('total_price_s').innerText)*${lcDiscount }){
...
}else
{
  	...
  	if(Number($('total_price').innerText)-Number(${fixeds})>=Number($('total_price_s').innerText)*${lcDiscount }){
		if(Number($('total_price').innerText)!=Number($('total_price_s').innerText)*${lcDiscount }){
		    if (!confirm("<jecs:locale key='insufficient.error'/>")){ 
				return false;
			}
...
<jecs:power powerCode="addPoMemberSSubFOrderLC">
				<input data-inline="true" type="submit" class="button" name="save"  onclick="return nextSave();" value="<jecs:locale key="operation.button.save"/>" />
</jecs:power>
</c:if>
...
<jecs:power powerCode="deletePoMemberSSubFOrderLC">
				<input data-inline="true" type="submit" class="button" name="delete" onclick="bCancel=true;return confirmDelete(this.form,'PoMemberSSubFOrderLC')" value="<jecs:locale key="operation.button.delete"/>" />
</jecs:power>
</c:if>
...
<script>
  ...
  function amt_count(step,type){
	if(step==2){
		...
		if(type=="fixed")
		{
		     document.getElementById("fixed").value=nPriceAmt;
		}
    	...
    	$('total_price_pre').innerHTML=Math.round((nPriceAmt-Number(document.getElementById("fixed").value))/${lcDiscount },2);
    	}else
    	{
    
    	      	$('total_price_pre').innerHTML=Math.round((nPriceAmt-Number(${fixeds}))/${lcDiscount },2);
    	}
...
<form:form commandName="jpoMemberOrder" method="post" action="editJpoMemberSSubFOrderLC.html" id="jpoMemberOrderForm">
...
<c:if test='${param.strAction=="editPoMemberSSubFOrderLC"}'>
    <tr><th>
    ...
    <td align="left">
    	...
    	<c:if test='${param.strAction=="addPoMemberSSubFOrderLC"}'>
...
<c:if test="${sessionLogin.userType=='M' }">
    ...
    <%-- <tr><th>
        <jecs:label key="jpoMemberOrder.agree"/>1
    </th>
    <td align="left">
    	<c:if test="${empty jpoMemberOrder.moId }">
    ...
    <tr><th>
        <jecs:label key="jpoMemberOrder.agree"/>2
    </th>
    <td align="left">
    	<c:if test="${empty jpoMemberOrder.moId }">
    		<input type="checkbox" name="subFAgree" id="subFAgree" /><jecs:locale key="jpoMemberOrder.agree.subf.msg"/>
    	</c:if>
    	<c:if test="${not empty jpoMemberOrder.moId }">
    		<input type="checkbox" name="subFAgree" id="subFAgree" checked/><jecs:locale key="jpoMemberOrder.agree.subf.msg"/>
    	</c:if>
    </td></tr>
</c:if>
	...
	<div class="searchBar">
		<input data-inline="true" type="button" class="button" name="step1Next"  onclick="stepNext1();" value="<jecs:locale key="button.next"/>" />
		<input data-inline="true" type="button" class="button" name="back"  onclick="window.location.href='jpoMemberSSubFOrders.html?needReload=1'" value="<jecs:locale key="operation.button.return"/>" />
	</div>
</div>
<script>
function stepBack1(){
...
function stepNext2(){
	if(Number($('total_pv').innerText)>=${pvamt}){
		$('step2').style.display='none';
		$('step3').style.display='inline';
	}else{
		alert('<jecs:locale key="pvamt.notEnough"/>');
	}
}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td><br />
    	<jecs:code listCode="pmproduct.productcategory" value="${jpms.key }"/>,<jecs:label key="pd.price"/>${jpms.value.productInfo.substoreFpPrice },<jecs:label key="busi.direct.pv"/>${jpms.value.productInfo.substoreFpPv }<br />
		<br />
				...
				<tr>
					...
					<c:forEach items="${jpms.value.size }" var="size">
						...
						<c:if test="${not empty jpms.value.jpmProductSales[style.PRODUCT_STYLE] && not empty jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE] }">
						    <c:if test="${jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE].jpmProduct.productNo!='NLC000000000CN0'}">
						    ...
						    <c:if test="${jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE].jpmProduct.productNo=='NLC000000000CN0' }">
						    1<input type='hidden' name='qty' value='1' size='2' id=qty<c:out value="${jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE].uniNo }2" />>
						    </c:if> 
						    ...
						    <script>
								...
								aSub2[<c:out value="${jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE].uniNo }" />2]['qty']='<c:if test="${jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE].jpmProduct.productNo!='NLC000000000CN0'}">0</c:if><c:if test="${jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE].jpmProduct.productNo=='NLC000000000CN0'}">1</c:if>';
								aSub2[<c:out value="${jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE].uniNo }" />2]['price']='<c:out value="${jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE].substoreFpPrice }" />';
								aSub2[<c:out value="${jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE].uniNo }" />2]['pv']='<c:out value="${jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE].substoreFpPv }" />';
							</script><%--
							<jecs:label key="pd.price"/>${jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE].substoreFpPrice }<br/>
							<jecs:label key="busi.direct.pv"/>${jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE].substoreFpPv }
						--%></c:if>
...
<c:if test="${varS % 2 == 1  }">
	...
	<jecs:locale key="busi.order.amount"/><span id='total_price'>0</span>,<span><input type="hidden" value="" id="fixed"/></span><jecs:locale key="poMemberOrder.pvAmt"/><span id='total_pv'>0</span>
	<div class="searchBar">
		<input data-inline="true" type="button" class="button" name="step1Back"  onclick="stepBack1();" value="<jecs:locale key="register.perous"/>" />
		<input data-inline="true" type="button" class="button" name="step2Next"  onclick="stepNext2();" value="<jecs:locale key="button.next"/>" />
		<input data-inline="true" type="button" class="button" name="back"  onclick="window.location.href='jpoMemberSSubFOrders.html?needReload=1'" value="<jecs:locale key="operation.button.return"/>" />
	</div>
</div>
<script>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td><br />
    	<jecs:code listCode="pmproduct.productcategory" value="${jpms.key }"/>,<jecs:label key="pd.price"/>${jpms.value.productInfo.substoreFpPrice },<jecs:label key="busi.direct.pv"/>0<br />
		<br />
		<table id="tablemain" class="tree_grid" border="0" cellspacing="0" cellpadding="0" width="40%">
			<thead>
				<tr>
				...
				<tr>
					...
					<c:forEach items="${jpms.value.size }" var="size">
						...
						<c:if test="${not empty jpms.value.jpmProductSales[style.PRODUCT_STYLE] && not empty jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE] }">
						    ...
						    <c:if test="${jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE].jpmProduct.productNo=='NLC000000000CN0' }">
						    0<input type='hidden' name='qty' value='0' size='2' id=qty<c:out value="${jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE].uniNo }3" />>
						    </c:if> 
						    ...
						    <script>
								...
								aSub3[<c:out value="${jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE].uniNo }" />3]['price']='<c:out value="${jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE].substoreFpPrice }" />';
...
<c:if test="${varS % 2 == 1  }">
	...
	<div class="searchBar">
		<input data-inline="true" type="button" class="button" name="step1Back"  onclick="stepBack2();" value="<jecs:locale key="register.perous"/>" />
