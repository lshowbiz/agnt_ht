...
<form:form commandName="fiProductPointTemp" method="post"
	action="editFiProductPointTemp.html" onsubmit="return validateOthers(this)"
	id="fiProductPointTempForm">
		...
		<tbody class="window">
			<tr class="edit_tr">
				...
				<td>
					...
					<span id="userName">${fiProductPointTemp.sysUser.userName }</span>
				</td>
				<th>
					<span class="text-font-style-tc">
				...
				<td>
					<span class="textbox">
						<jecs:list listCode="fiproductpointtemp.productpointtype"
						name="productPointType" id="productPointType"
						value="${fiProductPointTemp.productPointType}" defaultValue="1" styleClass="textbox-text"/>
					</span>
				</td>			
			</tr>
			<tr class="edit_tr">
				...
				<td>
					<span class="textbox">
						...
						id="dealType" value="${fiProductPointTemp.dealType}" defaultValue="A" styleClass="textbox-text"/>
					</span>
				</td>
				<th>
					<span class="text-font-style-tc">
							...
							<c:forEach items="${moneyTypes}" var="moneyTypeVar">
								<c:if
									...
									<option value="${moneyTypeVar.key }"
										<c:if test="${moneyTypeVar.key==fiProductPointTemp.moneyType || (empty fiProductPointTemp.moneyType && moneyTypeVar.key==1)}"> selected</c:if>>
			...
			<tr>
				<td class="command" colspan="4" align="center">
					...
					<c:if test="${not empty fiProductPointTemp.tempId}">
						<jecs:power powerCode="deleteFiProductPointTempJJ">
							<input type="submit" class="btn btn_sele" name="delete"
								onclick="bCancel=true;return confirmDelete(this.form,'FiProductPointTemp')"
...
<v:javascript formName="fiProductPointTemp" cdata="false"
