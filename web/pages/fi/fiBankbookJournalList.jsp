...
<form action="fiBankbookJournals.html" method="get"
	...
	<ec:table tableId="fiBankbookJournalListTable"
		...
		action="${pageContext.request.contextPath}/fiBankbookJournals.html"
		...
		<ec:row>
			...
			<ec:column property="balance" title="fiBankbookJournal.balance"
				...
				<c:if test="${not empty fiBankbookJournal.balance}">
					<fmt:formatNumber
						value="${fiBankbookJournal.balance}"
						type="number" pattern="###,###,##0.00" />
				</c:if>
			</ec:column>
			<c:if test="${sessionLogin.userType=='C' }">
...
function viewFiBankbookJournal(journalId){
	<jecs:power powerCode="viewFiBankbookJournal">
		window.location="fiViewBankbookJournal.html?strAction=viewFiBankbookJournal&journalId="+journalId;
	</jecs:power>
}
</script>
