...
<title><jecs:locale key="jfiPayAlipayDetail.title"/></title>
<content tag="heading"><jecs:locale key="jfiPayAlipayDetail.heading"/></content>
<script language="javascript" src="scripts/validate.js"></script>
<script type="text/javascript" src="<c:url value='/dwr/util.js'/>" ></script> 
<script type="text/javascript" src="<c:url value='/dwr/engine.js'/>" ></script>
<script type="text/javascript" src="<c:url value='/dwr/interface/alipayUtil.js'/>" ></script>
<script type="text/javascript" src="<c:url value='/dwr/interface/Alipay.js'/>" ></script>

<form id="jfiPayAlipay" method="GET" action="" onsubmit="return validateOthers(this);">
<table class='detail' width="100%">
    ...
    <td align="left">
    	<jecs:list listCode="alipay.bank.id" name="bankId" id="bankId" value="ICBCB2C" defaultValue="ICBCB2C"/>
    </td></tr>
	...
	<c:if test='${empty param.orderId}'>
		if($('orderAmount1').value>0){
			if(isFormPosted()){
				...
				alipay.defaultbank = $('bankId').value;
				alipay.total_fee = $('orderAmount1').value;
				alipay.extra_common_param = '${sessionLogin.userCode }';
				alipay.out_trade_no = '${param.orderId}';
				alipayUtil.getAlipay(alipay,${flag},callBack);
			}
		}
	</c:if>
