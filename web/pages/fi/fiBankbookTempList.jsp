...
<title><jecs:locale key="fiBankbookTempList.title" /></title>
<content tag="heading">
...
<form action="" method="get" name="fiBankbookTempSearchForm"
	...
	<div class="searchBar">
		...
		<div class="new_searchBar">
			...
			<jecs:power powerCode="addFiBankbookTempJJ">
				<button name="add" class="btn btn_ins" type="button"
					onclick="location.href='<c:url value="/editFiBankbookTemp.html"/>?strAction=addFiBankbookTempJJ'">
					<jecs:locale key="member.new.num"/>
				</button>
			</jecs:power>
			<jecs:power powerCode="multiDeleteFiBankbookTempJJ">
				<button name="delete" class="btn btn_ins" type="button"
					onclick="doMultiDeleteFiBankbookTemp($('fiBankbookTempSearchForm'));">
					<jecs:locale key="operation.button.delete"/>
				</button>
			</jecs:power>
			<jecs:power powerCode="importFiBankbookTempJJ">
				<button name="import" class="btn btn_ins" type="button"
					onclick="location.href='<c:url value="/importFiBankbookTemp.html"/>'">
					<jecs:locale key="button.import"/>
				</button>
			</jecs:power>
		</div>
	</div>
	<input type="hidden" name="strAction" value="listfiBankbookTempsJJ" />
...
<ec:table tableId="fiBankbookTempListTable" items="fiBankbookTempList"
	...
	action="${pageContext.request.contextPath}/fiBankbookTemps.html"
	...
	<ec:row highlightRow="false">
		...
		<ec:column property="createTime" title="pd.createTime" >
			<fmt:formatDate value="${fiBankbookTemp.createTime }" pattern="yyyy-MM-dd HH:mm:ss"/>
		</ec:column>
		<ec:column property="notes" title="bdBounsDeduct.summary"
			sortable="false">
			<jecs:substr key="${fiBankbookTemp.notes }" length="10"/> 
		</ec:column>
		<ec:column property="description" title="bdBounsDeduct.description"
			sortable="false">
			<jecs:substr key="${fiBankbookTemp.description }" length="10"/> 
		</ec:column>
		...
		<ec:column property="checkerName" title="pd.confirmUserNo" width="100">
			<font color="green">${fiBankbookTemp.checkerName }</font>&nbsp;<fmt:formatDate value="${fiBankbookTemp.checkeTime }" pattern="yyyy-MM-dd HH:mm:ss"/>
		</ec:column>
		<ec:column property="1" title="title.operation" sortable="false"
			...
			<jecs:power powerCode="editFiBankbookTemp">
				<c:if test="${fiBankbookTemp.status!=2}">
					<a
						href="editFiBankbookTemp.html?tempId=${fiBankbookTemp.tempId}&strAction=editFiBankbookTemp">
...
function doMultiDeleteFiBankbookTemp(theForm){
	...
	theForm.strAction.value="multiDeleteFiBankbookTempJJ";
	if(isFormPosted()){
		theForm.submit();
	}
}
