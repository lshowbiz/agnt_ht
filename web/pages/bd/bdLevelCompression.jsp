...
<div class="searchBar">
	...
	<div class="new_searchBar" style="margin-left: 65px">
		...
		<button type="button" class="btn btn_ins" name="expandAll"  onclick="tree.expandAll();">
...
<tr>
	<td><div>会员编号 - [级别] - [死点] - [冻结]  - [退出状态]</div></td>
</tr>
<tr>
...
<c:if test="${not empty bdlist}">
	...
	<c:forEach items="${bdlist}" var="curBd" varStatus="status">
		...
		<c:if test="${status.count == 1}">
			tree.N["0_<c:out value="${curBd.user_code}"/>"] = "T:<font color=#0066CC><c:out value="${curBd.user_code}"/> - [<jecs:code listCode="member.level" value="${curBd.member_level}"/>] - [死点：<jecs:code listCode="yesno" value="${curBd.active}"/>] -  [冻结：<jecs:code listCode="yesno" value="${curBd.freeze_status}"/>] - [退出：<c:if test="${not empty curBd.exit_date}">是</c:if><c:if test="${ empty curBd.exit_date}">否</c:if>] </font> ;";
		</c:if>
		<c:if test="${status.count != 1}">
			
			tree.N["<c:out value="${curBd.recommend_no}"/>_<c:out value="${curBd.user_code}"/>"] = "T:<font color=#0066CC><c:out value="${curBd.user_code}"/> - [<jecs:code listCode="member.level" value="${curBd.member_level}"/>] - [死点：<jecs:code listCode="yesno" value="${curBd.active}"/>] -  [冻结：<jecs:code listCode="yesno" value="${curBd.freeze_status}"/>] - [退出：<c:if test="${not empty curBd.exit_date}">是</c:if><c:if test="${ empty curBd.exit_date}">否</c:if>]  </font> ;";
