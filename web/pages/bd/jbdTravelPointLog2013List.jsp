...
<jecs:title key="pd.createTime"/>
	 <input id="startTime" name="startTime" type="text" size="8" maxlength="10" value="${param.startTime }"/>
		...
		<jecs:power powerCode="addJbdTravelPointLog2013">
			    <input type="button" class="button" style="margin-right: 5px"
			        onclick="location.href='<c:url value="/editJbdTravelPointLog2013.html"/>?strAction=addJbdTravelPointLog2013'"
...
<ec:table 
	tableId="jbdTravelPointLog2013ListTable"
	items="jbdTravelPointLog2013List"
	var="jbdTravelPointLog2013"
	action="${pageContext.request.contextPath}/jbdTravelPointLog2013s.html"
	width="100%"
	...
	rowsDisplayed="20" sortable="true" imagePath="./images/extremetable/*.gif">
				...
				<jecs:power powerCode="jbdTravelPointLog2013sExport">
				
	<ec:exportXls fileName="jbdTravelPointLog2013s.xls"></ec:exportXls>
				</jecs:power>
		...
		<ec:column property="usePoint" title="fibankbooktemp.dealtype.A" cell="com.joymain.jecs.util.ectable.EcTableCell">
			<c:if test="${jbdTravelPointLog2013.dealType=='A'}">${jbdTravelPointLog2013.usePoint}</c:if>
			<c:if test="${jbdTravelPointLog2013.dealType=='D'}">0</c:if>
		</ec:column>
		<ec:column property="usePoint" title="fibankbooktemp.dealtype.D" cell="com.joymain.jecs.util.ectable.EcTableCell">
			<c:if test="${jbdTravelPointLog2013.dealType=='D'}">${jbdTravelPointLog2013.usePoint}</c:if>
			<c:if test="${jbdTravelPointLog2013.dealType=='A'}">0</c:if>
		</ec:column>
