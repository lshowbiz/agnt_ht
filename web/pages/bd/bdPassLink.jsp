...
<tr>
	<td><div>会员编号 - 会员姓名 - [合格奖衔] - [整组业绩] - [审核日期] - 状态(授权/停权)</div></td>
</tr>
<tr>
...
<c:if test="${not empty bdlist}">
	...
	<c:forEach items="${bdlist}" var="curBd" varStatus="status">
		...
		<c:if test="${status.count == 1}">
			tree.N["0_<c:out value="${curBd.user_code}"/>"] = "T:<font color=#0066CC><c:out value="${curBd.user_code} - ${curBd.mi_user_name}"/> - [<jecs:code listCode="pass.star.zero" value="${curBd.pass_star}"/>] - "+${curBd.week_group_pv}+" -  [<fmt:formatDate value="${curBd.check_date }" pattern="yyyy-MM-dd"/>]  -  <c:if test="${curBd.state=='1'}">授权</c:if><c:if test="${curBd.state=='0'}">停权</c:if> </font> ;";
		</c:if>
		<c:if test="${status.count != 1}">
			
			tree.N["<c:out value="${curBd.link_no}"/>_<c:out value="${curBd.user_code}"/>"] = "T:<font color=#0066CC><c:out value="${curBd.user_code} - ${curBd.mi_user_name}"/>  - [<jecs:code listCode="pass.star.zero" value="${curBd.pass_star}"/>] - "+${curBd.week_group_pv}+" -  [<fmt:formatDate value="${curBd.check_date }" pattern="yyyy-MM-dd"/>]   -  <c:if test="${curBd.state=='1'}">授权</c:if><c:if test="${curBd.state=='0'}">停权</c:if> </font> ;";
