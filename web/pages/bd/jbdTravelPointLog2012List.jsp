...
<jecs:title key="pd.createTime"/>
	 <input id="startTime" name="startTime" type="text" size="8" maxlength="10" value="${param.startTime }"/>
		...
		<jecs:power powerCode="addJbdTravelPointLog2012">
			    <input type="button" class="button" style="margin-right: 5px"
			        onclick="location.href='<c:url value="/editJbdTravelPointLog2012.html"/>?strAction=addJbdTravelPointLog2012'"
...
<ec:table 
	tableId="jbdTravelPointLog2012ListTable"
	items="jbdTravelPointLog2012List"
	var="jbdTravelPointLog2012"
	action="${pageContext.request.contextPath}/jbdTravelPointLog2012s.html"
	width="100%"
	...
	rowsDisplayed="20" sortable="true" imagePath="./images/extremetable/*.gif">
				...
				<jecs:power powerCode="jbdTravelPointLog2012sExport">
				
	<ec:exportXls fileName="jbdTravelPointLog2012s.xls"></ec:exportXls>
				</jecs:power>
		...
		<ec:column property="usePoint" title="fibankbooktemp.dealtype.A" cell="com.joymain.jecs.util.ectable.EcTableCell">
			<c:if test="${jbdTravelPointLog2012.dealType=='A'}">${jbdTravelPointLog2012.usePoint}</c:if>
			<c:if test="${jbdTravelPointLog2012.dealType=='D'}">0</c:if>
		</ec:column>
		<ec:column property="usePoint" title="fibankbooktemp.dealtype.D" cell="com.joymain.jecs.util.ectable.EcTableCell">
			<c:if test="${jbdTravelPointLog2012.dealType=='D'}">${jbdTravelPointLog2012.usePoint}</c:if>
			<c:if test="${jbdTravelPointLog2012.dealType=='A'}">0</c:if>
		</ec:column>
