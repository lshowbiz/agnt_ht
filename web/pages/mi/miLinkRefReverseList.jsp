...
<form action="" method="post" name="miMemberForm" id="miMemberForm">
	<div class="searchBar">
		<div class="new_searchBar">
			...
			<input name="memberNo" type="text" value="${param.memberNo }" size="10"/>
		</div>
		<div class="new_searchBar">
			...
			<%-- 
			<input name="search" type="submit" onclick='loading();' class="button" 
			...
			<c:if test="${sessionScope.sessionLogin.userType!='C'}">
				...
				<c:if test="${status.count != 1}">
					<c:if test="${empty treeIndex}">
						<c:set var="treeIndex" value="${fn:substring(mi.treeIndex,0,fn:length(mi.treeIndex)-2)}"/>
					</c:if>
					<c:if test="${sessionScope.sessionLogin.companyCode!='AA'}">
						<c:if test="${sessionScope.sessionLogin.companyCode==mi.jmiMember.companyCode}">
							tree.N["<c:out value="${treeIndex}"/>_<c:out value="${mi.treeIndex}"/>"] = "T:<font color=#0066CC><c:out value="${mi.userCode} - ${mi.jmiMember.petName}"/></font> - [ <font color='${cardType}'>${mi.jmiMember.cardType}</font> ] - <font color=#FF0000><fmt:formatNumber value="${(fn:length(mi.treeIndex)-rootTreeIndex)/2-1}" pattern="##"/></font>  <font color=#FF0000><jecs:locale key="${(not empty mi.jmiMember.exitDate)&&mi.jmiMember.sysUser.state==0?'login.Out':'' }"/></font> <jecs:code listCode="store.type" value="${mi.jmiMember.isstore }" />"+syt+";";
							<c:set var="treeIndex" value="${null}"/>
						</c:if>
					</c:if>
					<c:if test="${sessionScope.sessionLogin.companyCode=='AA'}">
						tree.N["<c:out value="${fn:substring(mi.treeIndex,0,fn:length(mi.treeIndex)-2)}"/>_<c:out value="${mi.treeIndex}"/>"] = "T:<font color=#0066CC><c:out value="${mi.userCode} - ${mi.jmiMember.petName}"/></font> - [ <font color='${cardType}'>${mi.jmiMember.cardType}</font> ] - <font color=#FF0000><fmt:formatNumber value="${(fn:length(mi.treeIndex)-rootTreeIndex)/2-1}" pattern="##"/></font>  <font color=#FF0000><jecs:locale key="${(not empty mi.jmiMember.exitDate)&&mi.jmiMember.sysUser.state==0?'login.Out':'' }"/></font> <jecs:code listCode="store.type" value="${mi.jmiMember.isstore }" />"+syt+";";
					</c:if>
				</c:if>
			</c:if>
			<c:if test="${sessionScope.sessionLogin.userType=='C'}">
				...
				<c:if test="${status.count != 1}">
					...
					<c:if test="${sessionScope.sessionLogin.companyCode=='AA'}">
						tree.N["<c:out value="${fn:substring(mi.treeIndex,0,fn:length(mi.treeIndex)-2)}"/>_<c:out value="${mi.treeIndex}"/>"] = "T:<font color=#0066CC><c:out value="${mi.userCode} - ${mi.jmiMember.miUserName} - ${mi.jmiMember.petName}"/></font> - [ <font color='${cardType}'>${mi.jmiMember.cardType}</font> ] [ <jecs:code listCode="member.level" value="${mi.jmiMember.memberLevel}"/> ] - <font color=#FF0000><fmt:formatNumber value="${(fn:length(mi.treeIndex)-rootTreeIndex)/2-1}" pattern="##"/></font>  <font color=#FF0000><jecs:locale key="${(not empty mi.jmiMember.exitDate)&&mi.jmiMember.sysUser.state==0?'login.Out':'' }"/><fmt:formatDate value="${(not empty mi.jmiMember.exitDate)&&mi.jmiMember.sysUser.state==0 ? mi.jmiMember.exitDate : '' }" pattern="yyyy-MM-dd"/></font> <jecs:code listCode="store.type" value="${mi.jmiMember.isstore }" /><font color=#FF0000><jecs:locale key="${mi.jmiMember.active=='1' ? 'is.active' : '' }"/></font><jecs:code listCode="memberno.zcw" value="${mi.jmiMember.userCode}"/> 冻结状态：<jecs:code listCode="yesno" value="${mi.jmiMember.freezeStatus}"/> "+state+";";
					</c:if>
				</c:if>
			</c:if>
			</c:forEach>
