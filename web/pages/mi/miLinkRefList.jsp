...
<div class="searchBar">
	<div class="new_searchBar">
		...
		<input size="10" name="memberNo" type="text" value="${empty param.memberNo ? memberNo : param.memberNo }"/>
	</div>
	<div class="new_searchBar">
	...
	<div class="new_searchBar">
		<button name="search" id="search" type="submit" class="btn btn_sele" onclick='loading();'>
...
<c:if test="${sessionScope.sessionLogin.userType!='C'}">
	<c:forEach items="${miLinkRefList}" var="mi" varStatus="status">
...
<c:if test="${sessionScope.sessionLogin.userType=='C'}">
	...
	<c:choose>
		...
		<c:if test="${status.count != 1}">
			...
			tree.N["<c:out value="${mi.jmiMember.linkNo}"/>_<c:out value="${mi.userCode}"/>"] = "T:<img src=images/newIcons/block_16.gif /> - <jecs:locale key="miMember.haiwaixian"/> <jecs:code listCode="store.type" value="${mi.jmiMember.isstore }"/> "+state+";";
			</c:if>
			<c:if test="${mi.userCode!='haiwaixian'}">
			tree.N["<c:out value="${mi.jmiMember.linkNo}"/>_<c:out value="${mi.userCode}"/>"] = "T:<font color=#0066CC><c:out value="${mi.userCode} - ${mi.jmiMember.miUserName} - ${mi.jmiMember.petName}"/></font> [ <font color='${cardType}'>${mi.jmiMember.cardType}</font> ] [ <jecs:code listCode="member.level" value="${mi.jmiMember.memberLevel}"/> ] - <font color=#FF0000><fmt:formatNumber value="${(fn:length(mi.treeIndex)-rootTreeIndex)/2}" pattern="##"/></font> <font color=#FF0000><jecs:locale key="${(not empty mi.jmiMember.exitDate)&&mi.jmiMember.sysUser.state==0?'login.Out':'' }"/><fmt:formatDate value="${(not empty mi.jmiMember.exitDate)&&mi.jmiMember.sysUser.state==0 ? mi.jmiMember.exitDate : '' }" pattern="yyyy-MM-dd"/></font>[<jecs:locale key="${(not empty mi.jmiMember.checkDate)?('logType.C'):('pd.createTime') }"/>:<fmt:formatDate value="${(not empty mi.jmiMember.checkDate)?(mi.jmiMember.checkDate):(mi.jmiMember.createTime) }" pattern="yyyy-MM-dd"/>] <jecs:code listCode="store.type" value="${mi.jmiMember.isstore }"/>   <font color=#FF0000><jecs:locale key="${mi.jmiMember.active=='1' ? 'is.active' : '' }"/></font>   <font color=#FF0000><jecs:locale key="${mi.jmiMember.freezeStatus=='1' ? 'miMember.freezestatus1' : '' }"/></font><jecs:code listCode="memberno.zcw" value="${mi.jmiMember.userCode}"/> "+state+";";
			</c:if>
		</c:if>
	</c:forEach>
</c:if>
