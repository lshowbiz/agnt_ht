...
MzTreeView.prototype.nodeInit = function(sourceIndex, parentId)
{
  ...
  this.node[this.index] =
  {
	...
	"folderIcon"  : this.getAttribute(source, "folderIcon"),
...
MzTreeView.prototype.nodeToHTML = function(node, AtEnd)
{
  ...
  if(isRoot && node.icon=="") node.icon = "root";
  if(node.url.indexOf("html")>0 || node.url.indexOf("htm")>0)  node.icon = "file";
  //if(node.icon=="" || typeof(this.icons[node.icon])=="undefined")
    //node.icon = HCN ? "folder" : "file";
  //增加功能:可自定义文件夹打开和关闭的图标 --
  if(HCN && !isRoot){
	  if(node.folderIcon=="" || typeof(this.icons[node.folderIcon])=="undefined"){
		  node.icon="folder";
	  }else{
		node.icon=node.folderIcon;
	  }
  }else{
	if(node.icon=="" || typeof(this.icons[node.icon])=="undefined"){
		node.icon="file";
  ...
  if ( typeof MzTreeViewTH != "undefined" ) HTMTH=MzTreeViewTH;
	...
	var divClassName="menu";
	var divOverClassName="menu_down";
	if(!isRoot){
		divClassName="submenu";
		divOverClassName="submenu_down";
	}
  var HTML = "<DIV noWrap='True' id='"+ this.name +"_link_"+ id +"' title='"+ node.hint +"' "+ " onfocus=\""+ this.name +".focusLink('"+ id +"')\" "
  			+"' class='"+divClassName+"' onmouseover='this.className=\""+divOverClassName+"\"' onmouseout='this.className=\""+divClassName+"\"'><NOBR id='"+this.name+"_nobr_"+id+"'>"+HTMTH;
  ...
  if( node.ctrl.length > 0 ) HTMCHK = "<input type='checkbox' name='"+ node.ctrl +"' class='checkbox' value='"+ sid +
  ...
  HTML += "<IMG "+
    ...
    "src='"+ this.icons[node.icon].src +"'>" + HTMCHK + node.T + HTMTD +"</NOBR></DIV>";
...
MzTreeView.prototype.overNode=function(obj,newClassName){
	if(obj.className!="submenu_active"){
...
MzTreeView.prototype.clickHandle = function(e)
{
  ...
  switch(e.tagName)
  {
    ...
    case "SPAN" :
      ...
      break;
    case "DIV" :
    	if(e.id && (e.className=="menu_down" || e.className=="submenu_down" || e.className=="submenu_active")) {
			this.focusClientNode(e.id.substr(e.id.lastIndexOf("_") + 1));
			if(this.onClickOpen){
				this.expand(e.id.substr(e.id.lastIndexOf("_") + 1));
			}
...
MzTreeView.prototype.setRoot  = function(set_parent_id){
	  this.node["0"] =
	  {
		"id": set_parent_id,
		"path": set_parent_id,
	...
	if(tcn.parentId != "0")
	{
	  ...
	  for(var i=0; i<pnode.childNodes.length; i++)
	  {
	    ...
	    var ckboxID = this.name +"_checbox_"+ pnode.childNodes[i].id;
			if(this.getElementById(ckboxID)!=null){
	  ...
	  var pckboxid = this.name +"_checbox_"+ pnode.id
		if(this.getElementById(pckboxid)!=null)
		{
