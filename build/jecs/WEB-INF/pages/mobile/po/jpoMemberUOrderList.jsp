...
<script type="text/javascript">
   	function editJpoMemberUOrder(moId){
	 	<jecs:power powerCode="editPoMemberUOrder">
			location="${ctx}/editJpoMemberUOrder.html?strAction=editPoMemberUOrder&moId="+moId;
		</jecs:power>
	}
</script>
...
<jecs:power powerCode="addPoMemberUOrder">
	<c:if test="${sessionLogin.upGrade=='1'}">
		<c:if test="${sessionScope.sessionLogin.userType=='C'}">
			<input type="button" 
			onclick="location='${ctx}/editJpoMemberUOrder.html?strAction=addPoMemberUOrder'" value="<jecs:locale key="member.new.num"/>" />
		</c:if>
		...
		<c:if test="${sessionScope.sessionLogin.userType!='C'}">
			<c:if test="${fn:substring(sessionScope.sessionLogin.jmiMember.jmiRecommendRef.treeIndex,0,36)=='00100000000000000b00n00000u003000013'}">
				<c:if test="${sessionScope.sessionLogin.companyCode=='CN' }">
					<input type="button" 
					onclick="location='${ctx}/editJpoMemberUOrderLC.html?strAction=addPoMemberUOrderLC'" value="<jecs:locale key="member.new.num"/>" />
				</c:if>
				<c:if test="${sessionScope.sessionLogin.companyCode!='CN' }">
					<input type="button" 
					onclick="location='${ctx}/editJpoMemberUOrder.html?strAction=addPoMemberUOrder'" value="<jecs:locale key="member.new.num"/>" />
	  ...
	  <li>
	    ...
	    <c:if test="${jpoMemberOrder.status=='1' && jpoMemberOrder.isPay!='1' && jpoMemberOrder.submitStatus=='1'}">
			<c:if test="${jpoMemberOrder.productType!='LC' }">
		  		<jecs:power powerCode="editPoMemberUOrder">
		  		<p class="pPadding" onclick="editJpoMemberUOrder('${jpoMemberOrder.moId}')">
			...
			<c:if test="${jpoMemberOrder.productType=='LC' }">
		  		...
		  		<p class="pPadding" onclick="location='editJpoMemberUOrderLC.html?strAction=editPoMemberUOrderLC&moId=${jpoMemberOrder.moId}'">
				编辑<img src="${ctx}/images/newIcons/pencil_16.gif" /> 
				</p>
				</jecs:power>
			</c:if>
