...
<script>
   	function editJpoMemberSROrder(moId){
   		<jecs:power powerCode="editPoMemberSROrder">
			location="editJpoMemberSROrder.html?strAction=editPoMemberSROrder&moId="+moId;
		</jecs:power>
	}
</script>
...
<jecs:power powerCode="addPoMemberSROrder">
	<c:if test="${sessionScope.sessionLogin.userType=='C'}">
		<input type="button" onclick="location='${ctx}/editJpoMemberSROrder.html?strAction=addPoMemberSROrder'" value="<jecs:locale key="member.new.num"/>" />
	</c:if>
	...
	<c:if test="${sessionScope.sessionLogin.userType!='C'}">
		<c:if test="${fn:substring(sessionScope.sessionLogin.jmiMember.jmiRecommendRef.treeIndex,0,36)=='00100000000000000b00n00000u003000013'}">
			<c:if test="${sessionScope.sessionLogin.companyCode=='CN' }">
				<input type="button" onclick="location='${ctx}/editJpoMemberSROrderLC.html?strAction=addPoMemberSROrderLC'" value="<jecs:locale key="member.new.num"/>" />
			</c:if>
			<c:if test="${sessionScope.sessionLogin.companyCode!='CN' }">
				<input type="button" onclick="location='${ctx}/editJpoMemberSROrder.html?strAction=addPoMemberSROrder'" value="<jecs:locale key="member.new.num"/>" />
			</c:if>
	  ...
	  <li>
	    ...
	    <c:if test="${jpoMemberOrder.status=='1' && jpoMemberOrder.isPay!='1' && jpoMemberOrder.submitStatus=='1'}">
			<c:if test="${jpoMemberOrder.productType!='LC' }">
		   		<jecs:power powerCode="editPoMemberSROrder">
		   		<p class="pPadding" onclick="editJpoMemberSROrder('${jpoMemberOrder.moId}')">
			...
			<c:if test="${jpoMemberOrder.productType=='LC' }">
		   		...
		   		<p class="pPadding" onclick="location='editJpoMemberSROrderLC.html?strAction=editPoMemberSROrderLC&moId=${jpoMemberOrder.moId}'">
				编辑<img src="${ctx}/images/newIcons/pencil_16.gif" /> 
				</p>
				</jecs:power>
			</c:if>
