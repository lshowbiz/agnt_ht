...
<input data-inline="true" type="button" class="button" name="addLC"  
	onclick="window.location.href='editJpoMemberSROrderLC.html?strAction=addPoMemberSROrderLC'" 
	value="兰超" />
</c:if>
</jecs:power>
...
<jecs:power powerCode="deletePoMemberSROrder">
	<input data-inline="true" type="submit" class="button" name="delete" onclick="bCancel=true;return confirmDelete(this.form,'PoMemberSROrder')" value="<jecs:locale key="operation.button.delete"/>" />
</jecs:power>
</c:if>
...
<form:form commandName="jpoMemberOrder" method="post" action="editJpoMemberSROrder.html" onsubmit="return onSubmitCheck(this);" id="jpoMemberOrderForm">
...
<c:if test='${param.strAction=="editPoMemberSROrder"}'>
    <tr><th>
    ...
    <td align="left">
    	...
    	<c:if test='${param.strAction=="addPoMemberSROrder"}'>
...
<c:if test="${sta.count!=1}">
     <div id="tabCot_product_${sta.count}" class="tabCot"  style="display: none;">
       ...
       <%--  <td>  <jecs:locale  key="busi.product.pic"/></td>
        ...
        <td>${item.jpmProduct.productNo}
        	<br/>${item.productName}</td>
        <td>${item.storeMpPrice}</td>
          <td>${item.storeMpPv}</td>
          <td align="left">
            <input type='text' class='text medium' name='qty' value='0' size='3' onchange="set_amt(this,'${item.uniNo }');" id=qty<c:out value="${item.uniNo }" />>
				    ...
				    <script>
						...
						aSub[<c:out value="${item.uniNo }" />]['price']='<c:out value="${item.storeMpPrice }" />';
						aSub[<c:out value="${item.uniNo }" />]['pv']='<c:out value="${item.storeMpPv }" />';
					</script>
          </td>
