...
<jecs:power powerCode="addPoMemberSROrderLC">
...
<jecs:power powerCode="deletePoMemberSROrderLC">
				<input data-inline="true" type="submit" class="button" name="delete" onclick="bCancel=true;return confirmDelete(this.form,'PoMemberSROrderLC')" value="<jecs:locale key="operation.button.delete"/>" />
</jecs:power>
</c:if>
...
<form:form commandName="jpoMemberOrder" method="post" action="editJpoMemberSROrderLC.html" id="jpoMemberOrderForm">
...
<c:if test='${param.strAction=="editPoMemberSROrderLC"}'>
    <tr><th>
    ...
    <td align="left">
    	...
    	<c:if test='${param.strAction=="addPoMemberSROrderLC"}'>
...
<c:if test="${sessionLogin.userType=='M' }">
	...
	<div class="searchBar">
		...
		<input data-inline="true" type="button" class="button" name="back"  onclick="window.location.href='jpoMemberSROrders.html?needReload=1'" value="<jecs:locale key="operation.button.return"/>" />
	</div>
</div>
<script>
function stepBack1(){
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <td><br />
    	<jecs:code listCode="pmproduct.productcategory" value="${jpms.key }"/>,<jecs:label key="pd.price"/>${jpms.value.productInfo.storeMpPrice },<jecs:label key="busi.direct.pv"/>${jpms.value.productInfo.storeMpPv }<br />
		<br />
					...
					<c:forEach items="${jpms.value.size }" var="size">
						...
						<c:if test="${not empty jpms.value.jpmProductSales[style.PRODUCT_STYLE] && not empty jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE] }">
						    ...
						    <script>
								...
								aSub2[<c:out value="${jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE].uniNo }" />2]['price']='<c:out value="${jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE].storeMpPrice }" />';
								aSub2[<c:out value="${jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE].uniNo }" />2]['pv']='<c:out value="${jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE].storeMpPv }" />';
							</script><%--
							<jecs:label key="pd.price"/>${jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE].storeMpPrice }<br/>
							<jecs:label key="busi.direct.pv"/>${jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE].storeMpPv }
						--%></c:if>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <td><br />
    	<jecs:code listCode="pmproduct.productcategory" value="${jpms.key }"/>,<jecs:label key="pd.price"/>${jpms.value.productInfo.storeMpPrice },<jecs:label key="busi.direct.pv"/>0<br />
		<br />
		<table id="tablemain" class="tree_grid" border="0" cellspacing="0" cellpadding="0" width="40%">
			<thead>
				<tr>
					...
					<c:forEach items="${jpms.value.size }" var="size">
						...
						<c:if test="${not empty jpms.value.jpmProductSales[style.PRODUCT_STYLE] && not empty jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE] }">
						    ...
						    <script>
								...
								aSub3[<c:out value="${jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE].uniNo }" />3]['price']='<c:out value="${jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE].storeMpPrice }" />';
...
<c:if test="${varS % 2 == 1  }">
	...
	<div class="searchBar">
		...
		<c:out data-inline="true" value="${buttons}" escapeXml="false"/>
		<input data-inline="true" type="button" class="button" name="back"  onclick="window.location.href='jpoMemberSROrders.html?needReload=1'" value="<jecs:locale key="operation.button.return"/>" />
	</div>
</div>
</form:form>
