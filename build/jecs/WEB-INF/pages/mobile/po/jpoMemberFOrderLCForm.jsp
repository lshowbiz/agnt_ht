...
<c:set var="buttons">
	...
	<jecs:power powerCode="addPoMemberFOrderLC">
		...
		<input data-inline="true" type="submit" name="save"  onclick="return nextSave();" value="<jecs:locale key="operation.button.save"/>" />
	</jecs:power>
	</c:if>
	...
	<jecs:power powerCode="deletePoMemberFOrderLC">
		<input data-inline="true" type="submit" name="delete" onclick="bCancel=true;return confirmDelete(this.form,'PoMemberFOrderLC')" value<jecs:localege key="operation.button.delete"/>" />
	</jecs:power>
	</c:if>
</c:set>
...
<form:form commandName="jpoMemberOrder" method="post" action="editJpoMemberFOrderLC.html" id="jpoMemberOrderForm">
...
<c:if test='${param.strAction=="editPoMemberFOrderLC"}'>
    <tr><th>
    ...
    <td align="left">
    	...
    	<c:if test='${param.strAction=="addPoMemberFOrderLC"}'>
...
<c:if test="${sessionLogin.userType=='M' }">
	...
	<!-- 下一步，返回 -->
	<div class="searchBar">
		<input data-inline="true" type="button" name="step1Next"  onclick="stepNext1();" value="<jecs:locale key="button.next"/>" />
		<input data-inline="true" type="button" name="back"  onclick="window.location.href='jpoMemberFOrders.html?needReload=1'" value="<jecs:locale key="operation.button.return"/>" />
	</div>
</div><!-- id="step1" END -->
...
function stepNext2(){
    var pv=parseInt("${levelPV}");
    var pagePv = parseInt($('total_pv').innerHTML);
    if(pagePv<pv)
    {
      alert('<jecs:locale key="samount.levelPv"/>${levelPV}pv');
      return false;
    }else
    {
...
<c:forEach items="${jpmProductSales }" var="jpms" varStatus="varStatus">
	...
	<%-- <c:if test="${varStatus.count % 2 == 1  }"></c:if> --%>
		...
		<br />
		<table id="tablemain" class="tree_grid" border="0" cellspacing="0" cellpadding="0" width="100%">
			<thead>
				<tr>
					<th>size</th>
...
<c:if test="${varS % 2 == 1  }">
	...
	<div class="searchBar">
		<input data-inline="true" type="button" name="step1Back"  onclick="stepBack1();" value="<jecs:locale key="register.perous"/>" />
		<input data-inline="true" type="button" name="step2Next"  onclick="stepNext2();" value="<jecs:locale key="button.next"/>" />
		<input data-inline="true" type="button" name="back"  onclick="window.location.href='jpoMemberFOrders.html?needReload=1'" value="<jecs:locale key="operation.button.return"/>" />
	</div>
...
<c:if test="${varS % 2 == 1  }">
	...
	<div class="searchBar">
		<input data-inline="true" type="button" name="step1Back"  onclick="stepBack2();" value="<jecs:locale key="register.perous"/>" />
