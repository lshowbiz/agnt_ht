...
<jecs:power powerCode="addPoMemberRSOrder">
				<input data-inline="true" type="submit" class="button" name="save" id="save" onclick="bCancel=false" value="<jecs:locale key="operation.button.save"/>" />
</jecs:power>
</c:if>

<jecs:power powerCode="addPoMemberRSOrderLC">
<c:if test='${empty jpoMemberOrder.moId}'>
		<input data-inline="true" type="button" class="button" name="addLC"  onclick="window.location.href='editJpoMemberRSOrderLC.html?strAction=addPoMemberRSOrderLC'" value="<jecs:locale key="operation.button.addLC"/>" />
</c:if>
</jecs:power>
...
<jecs:power powerCode="deletePoMemberRSOrder">
				<input data-inline="true" type="submit" class="button" name="delete" onclick="bCancel=true;return confirmDelete(this.form,'PoMemberRSOrder')" value="<jecs:locale key="operation.button.delete"/>" />
</jecs:power>
</c:if>

<input data-inline="true" type="button" class="button" name="back"  onclick="window.location.href='jpoMemberRSOrders.html?needReload=1'" value="<jecs:locale key="operation.button.return"/>" />
...
<form:form commandName="jpoMemberOrder" method="post" action="editJpoMemberRSOrder.html" onsubmit="return onSubmitCheck(this);" id="jpoMemberOrderForm">
...
<c:if test='${param.strAction=="editPoMemberRSOrder"}'>
    <tr><th>
    ...
    <td align="left">
    	...
    	<c:if test='${param.strAction=="addPoMemberRSOrder"}'>
          ...
          <td align="left">
            <input type='text' class='text medium' name='qty' value='0' size='3' onchange="set_amt(this,'${item.uniNo }');" id=qty<c:out value="${item.uniNo }" />>
				    ...
				    <script>
						...
						aSub[<c:out value="${item.uniNo }" />]['price']='<c:out value="${item.mpPrice }" />';
						aSub[<c:out value="${item.uniNo }" />]['pv']='<c:out value="${item.mpPv }" />';
					</script>
          </td>
