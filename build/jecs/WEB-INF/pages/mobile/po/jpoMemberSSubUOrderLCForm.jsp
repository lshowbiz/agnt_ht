...
<jecs:power powerCode="addPoMemberSSubUOrderLC">
...
<jecs:power powerCode="editPoMemberSSubUOrderLC">
				<input data-inline="true" type="submit" class="button" name="save"  onclick="return nextSave();" value="<jecs:locale key="operation.button.save"/>" />
</jecs:power>
<jecs:power powerCode="deletePoMemberSSubUOrderLC">
				<input data-inline="true" type="submit" class="button" name="delete" onclick="bCancel=true;return confirmDelete(this.form,'PoMemberSSubUOrderLC')" value="<jecs:locale key="operation.button.delete"/>" />
</jecs:power>
</c:if>
...
<form:form commandName="jpoMemberOrder" method="post" action="editJpoMemberSSubUOrderLC.html" id="jpoMemberOrderForm">
...
<c:if test='${param.strAction=="editPoMemberSSubUOrderLC"}'>
    <tr><th>
    ...
    <td align="left">
    	...
    	<c:if test='${param.strAction=="addPoMemberSSubUOrderLC"}'>
...
<c:if test="${sessionLogin.userType=='M' }">
	...
	<div class="searchBar">
		...
		<input data-inline="true" type="button" class="button" name="back"  onclick="window.location.href='jpoMemberSSubUOrders.html?needReload=1'" value="<jecs:locale key="operation.button.return"/>" />
	</div>
</div>
<script>
function stepBack1(){
...
function stepNext2(){
	if(Number($('total_pv').innerText)>=${pv}){
		$('step2').style.display='none';
				...
				<tr>
					...
					<c:forEach items="${jpms.value.size }" var="size">
						...
						<c:if test="${not empty jpms.value.jpmProductSales[style.PRODUCT_STYLE] && not empty jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE] }">
						    ...
						    <c:if test="${jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE].jpmProduct.productNo=='NLC000000000CN0' }">
						    1<input type='hidden' name='qty' value='2' size='2' id=qty<c:out value="${jpms.value.jpmProductSales[style.PRODUCT_STYLE][size.PRODUCT_SIZE].uniNo }2" />>
						    </c:if> 
