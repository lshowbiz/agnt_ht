...
<script>
   	function editJpoMemberSFOrder(moId){
   		<jecs:power powerCode="editPoMemberSFOrder">
			location="${ctx}/editJpoMemberSFOrder.html?strAction=editPoMemberSFOrder&moId="+moId;
		</jecs:power>
	}
</script>
...
<jecs:power powerCode="addPoMemberSFOrder">
	<c:if test="${sessionScope.sessionLogin.userType=='C'}">
		<input type="button" onclick="location='${ctx}/editJpoMemberSFOrder.html?strAction=addPoMemberSFOrder'" value="<jecs:locale key="member.new.num"/>" />
	</c:if>
	<c:if test="${sessionScope.sessionLogin.userType!='C'}">
		<c:if test="${fn:substring(sessionScope.sessionLogin.jmiMember.jmiRecommendRef.treeIndex,0,36)=='00100000000000000b00n00000u003000013'}">
			<c:if test="${sessionScope.sessionLogin.companyCode=='CN' }">
				<input type="button" onclick="location='${ctx}/editJpoMemberSFOrderLC.html?strAction=addPoMemberSFOrderLC'" value="<jecs:locale key="member.new.num"/>" />
			</c:if>
			<c:if test="${sessionScope.sessionLogin.companyCode!='CN' }">
				<input type="button" onclick="location='${ctx}/editJpoMemberSFOrder.html?strAction=addPoMemberSFOrder'" value="<jecs:locale key="member.new.num"/>" />
			</c:if>
	  ...
	  <li>
	    ...
	    <c:if test="${jpoMemberOrder.status=='1' && jpoMemberOrder.isPay!='1' && jpoMemberOrder.submitStatus=='1'}">
			<c:if test="${jpoMemberOrder.productType!='LC' }">
		   		<jecs:power powerCode="editPoMemberSFOrder">
		   		<p class="pPadding" onclick="editJpoMemberSFOrder('${jpoMemberOrder.moId}')">
			...
			<c:if test="${jpoMemberOrder.productType=='LC' }">
		   		...
		   		<p class="pPadding" onclick="location='editJpoMemberSFOrderLC.html?strAction=editPoMemberSFOrderLC&moId=${jpoMemberOrder.moId}'">
				编辑<img src="${ctx}/images/newIcons/pencil_16.gif" /> 
				</p>
				</jecs:power>
			</c:if>
