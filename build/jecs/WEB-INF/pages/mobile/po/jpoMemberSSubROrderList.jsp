...
<script>
   	function editJpoMemberSSubROrder(moId){
   		<jecs:power powerCode="editPoMemberSSubROrder">
			location="editJpoMemberSSubROrder.html?strAction=editPoMemberSSubROrder&moId="+moId;
		</jecs:power>
	}
</script>
...
<jecs:power powerCode="addPoMemberSSubROrder">
	<c:if test="${sessionScope.sessionLogin.userType=='C'}">
		<input type="button" onclick="location='${ctx}/editJpoMemberSSubROrder.html?strAction=addPoMemberSSubROrder'" value="<jecs:locale key="member.new.num"/>" />
	</c:if>
	...
	<c:if test="${sessionScope.sessionLogin.userType!='C'}">
		<c:if test="${fn:substring(sessionScope.sessionLogin.jmiMember.jmiRecommendRef.treeIndex,0,36)=='00100000000000000b00n00000u003000013'}">
			<c:if test="${sessionScope.sessionLogin.companyCode=='CN' }">
				<input type="button" onclick="location='${ctx}/editJpoMemberSSubROrderLC.html?strAction=addPoMemberSSubROrderLC'" value="<jecs:locale key="member.new.num"/>" />
			</c:if>
	  ...
	  <li>
	    ...
	    <c:if test="${jpoMemberOrder.status=='1' && jpoMemberOrder.isPay!='1' && jpoMemberOrder.submitStatus=='1'}">
			<c:if test="${jpoMemberOrder.productType!='LC' }">
		   		<jecs:power powerCode="editPoMemberSSubROrder">
		   		<p class="pPadding" onclick="editJpoMemberSSubROrder('${jpoMemberOrder.moId}')">
			...
			<c:if test="${jpoMemberOrder.productType=='LC' }">
		   		...
		   		<p class="pPadding" onclick="location='editJpoMemberSSubROrderLC.html?strAction=editPoMemberSSubROrderLC&moId=${jpoMemberOrder.moId}'">
				编辑<img src="${ctx}/images/newIcons/pencil_16.gif" /> 
				</p>
				</jecs:power>
			</c:if>
