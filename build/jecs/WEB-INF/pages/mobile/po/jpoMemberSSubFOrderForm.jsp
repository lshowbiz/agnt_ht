...
<jecs:power powerCode="addPoMemberSSubFOrder">
	<input data-inline="true" type="submit" value="<jecs:locale key="operation.button.save"/>" />
</jecs:power>
</c:if>

<jecs:power powerCode="addPoMemberSSubFOrderLC">
<c:if test='${empty jpoMemberOrder.moId}'>
	<input data-inline="true" type="button" onclick="window.location.href='editJpoMemberSSubFOrderLC.html?strAction=addPoMemberSSubFOrderLC'" 
	value="兰超" />
</c:if>
</jecs:power>
...
<jecs:power powerCode="deletePoMemberSSubFOrder">
	<input data-inline="true" type="submit" onclick="return confirmDelete(this.form,'PoMemberSSubFOrder')" 
	value="<jecs:locale key="operation.button.delete"/>" />
</jecs:power>
</c:if>

<input data-inline="true" type="button" onclick="window.location.href='jpoMemberSSubFOrders.html?needReload=1'" 
...
<form:form commandName="jpoMemberOrder" method="post" action="editJpoMemberSSubFOrder.html" onsubmit="return onSubmitCheck(this);" id="jpoMemberOrderForm">
...
<c:if test='${param.strAction=="editPoMemberSSubFOrder"}'>
    <tr><th>
    ...
    <td align="left">
    	...
    	<c:if test='${param.strAction=="addPoMemberSSubFOrder"}'>
...
<c:if test="${sessionLogin.userType=='M' }">
    ...
    <td align="left">
    	<c:if test="${empty jpoMemberOrder.moId }">
    		<label for="subFAgree">同意</label>
    		<input type="checkbox" name="subFAgree" id="subFAgree" />
    	</c:if>
    	<c:if test="${not empty jpoMemberOrder.moId }">
    		<label for="subFAgree">同意</label>
    		<input type="checkbox" name="subFAgree" id="subFAgree" checked/>
    	</c:if>
    </td></tr>
</c:if>
...
<c:if test="${sta.count!=1}">
     <div id="tabCot_product_${sta.count}" class="tabCot"  style="display: none;">
       ...
       <%--  <td>  <jecs:locale  key="busi.product.pic"/></td>
        ...
        <%-- <td><c:if test="${not empty item.imageLink}">
        ...
        <td>${item.substoreFpPrice}</td>
          <td>${item.substoreFpPv}</td>
          <td align="left">
            <input type='text' class='text medium' name='qty' value='0' size='3' onchange="set_amt(this,'${item.uniNo }');" id=qty<c:out value="${item.uniNo }" />>
				    ...
				    <script>
						...
						aSub[<c:out value="${item.uniNo }" />]['price']='<c:out value="${item.substoreFpPrice }" />';
						aSub[<c:out value="${item.uniNo }" />]['pv']='<c:out value="${item.substoreFpPv }" />';
					</script>
          </td>
