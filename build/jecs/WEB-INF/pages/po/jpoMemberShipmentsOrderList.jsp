...
<form action="jpoMemberShipmentsOrders.html" method="get" name="searchForm" id="searchForm">
<div class="searchBar">
...
<div class="new_searchBar">
<jecs:title key="poMemberOrder.shipments"/>
<jecs:list name="isShipments" listCode="yesnoshipments" value="${param.isShipments}" defaultValue="" showBlankLine="true"/>
</div>
...
<button name="search" class="btn btn_long" type="button" onclick="javascript:$('submitType').value='2';readCheck();" >
	<jecs:locale key="button.audit.shipments"/>
</button>
...
<ec:table 
	...
	action="${pageContext.request.contextPath}/jpoMemberShipmentsOrders.html"
	width="100%"
	...
	rowsDisplayed="20" sortable="true" imagePath="./images/extremetable/*.gif">
		...
		<ec:column property="1" title="alCharacterKey.select1" sortable="false" styleClass="centerColumn">
			<c:if test="${jpoMemberOrder.locked!='1' && jpoMemberOrder.status=='1'  && jpoMemberOrder.isShipments!='1'}">
    	...
    	<ec:column property="status" title="pd.checkstatus" styleClass="centerColumn">
    		...
    		<ec:column property="isShipments" title="poMemberOrder.shipments" styleClass="centerColumn">
    		<jecs:code listCode="yesnoshipments" value="${jpoMemberOrder.isShipments}"/>
    	</ec:column>
...
function readCheck(poMemberOrderCheck){
	...
	if($('orderId').value !=''){
		if(confirm("<jecs:locale key="aiAgentShipments.confirm"/>")){
			if(isFormPosted()){
			$('searchForm').action = 'jpoMemberShipmentsOrders.html?strAction=shipmentsStr';
