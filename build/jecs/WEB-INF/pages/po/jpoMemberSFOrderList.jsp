...
<form action="jpoMemberSFOrders.html" method="get" name="searchForm"
	id="searchForm">
	<div class="searchBar">
			...
			<jecs:power powerCode="addPoMemberSFOrder">
				<c:if test="${sessionScope.sessionLogin.userType=='C'}">
					<button name="add" class="btn btn_ins" type="button"
						onclick="location.href='<c:url value="/editJpoMemberSFOrder.html?strAction=addPoMemberSFOrder"/>'">
				...
				<c:if test="${sessionScope.sessionLogin.userType!='C'}">
					<c:if
						...
						<c:if test="${sessionScope.sessionLogin.companyCode=='CN' }">
							<button name="add" class="btn btn_ins" type="button"
								onclick="location.href='<c:url value="/editJpoMemberSFOrderLC.html?strAction=addPoMemberSFOrderLC"/>'">
								<jecs:locale key="member.new.num" />
							</button>
						</c:if>
						<c:if test="${sessionScope.sessionLogin.companyCode!='CN' }">
...
<ec:table tableId="jpoMemberOrderListTable" items="jpoMemberOrderList"
	...
	action="${pageContext.request.contextPath}/jpoMemberSFOrders.html"
			...
			<c:if
				...
				<c:if test="${jpoMemberOrder.productType!='LC' }">
					<jecs:power powerCode="editPoMemberSFOrder">
						<img src="<c:url value='/images/newIcons/pencil_16.gif'/>"
							onclick="javascript:editJpoMemberSFOrder('${jpoMemberOrder.moId}')"
				...
				<c:if test="${jpoMemberOrder.productType=='LC' }">
					<jecs:power powerCode="editPoMemberSFOrderLC">
						<img src="<c:url value='/images/newIcons/pencil_16.gif'/>"
							onclick="javascript:window.location='editJpoMemberSFOrderLC.html?strAction=editPoMemberSFOrderLC&moId=${jpoMemberOrder.moId}'"
							style="cursor: pointer;" title="<jecs:locale key="button.edit"/>" />
					</jecs:power>
				</c:if>
			</c:if>
...
<script type="text/javascript">
	function editJpoMemberSFOrder(moId) {
		<jecs:power powerCode="editPoMemberSFOrder">
		window.location = "editJpoMemberSFOrder.html?strAction=editPoMemberSFOrder&moId="
				+ moId;
		</jecs:power>
	}
</script>
