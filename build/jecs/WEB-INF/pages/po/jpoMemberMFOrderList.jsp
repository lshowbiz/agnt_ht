...
<form action="jpoMemberMFOrders.html" method="get" name="searchForm" id="searchForm">
<div class="searchBar">
...
<jecs:power powerCode="addPoMemberMFOrder">
	<c:if test="${sessionScope.sessionLogin.userType=='C'}">
	<input name="add" class="button" type="button" onclick="location.href='<c:url value="/editJpoMemberMFOrder.html?strAction=addPoMemberMFOrder"/>'" value="<jecs:locale key="member.new.num"/>" />
	</c:if>
	<c:if test="${sessionScope.sessionLogin.userType!='C'}">
		<c:if test="${fn:substring(sessionScope.sessionLogin.jmiMember.jmiRecommendRef.treeIndex,0,36)=='00100000000000000b00n00000u003000013'}">
					<c:if test="${sessionScope.sessionLogin.companyCode=='CN' }">
						<input name="add" class="button" type="button" onclick="location.href='<c:url value="/editJpoMemberMFOrderLC.html?strAction=addPoMemberMFOrderLC"/>'" value="<jecs:locale key="member.new.num"/>" />
					</c:if>
					<c:if test="${sessionScope.sessionLogin.companyCode!='CN' }">
						<input name="add" class="button" type="button" onclick="location.href='<c:url value="/editJpoMemberMFOrder.html?strAction=addPoMemberMFOrder"/>'" value="<jecs:locale key="member.new.num"/>" />
					</c:if>
...
<ec:table 
	...
	action="${pageContext.request.contextPath}/jpoMemberMFOrders.html"
	width="100%"
		...
		<c:if test="${jpoMemberOrder.productType!='LC' }">
   		<jecs:power powerCode="editPoMemberMFOrder">
							<img src="<c:url value='/images/newIcons/pencil_16.gif'/>" 
								onclick="javascript:editJpoMemberMFOrder('${jpoMemberOrder.moId}')"
								style="cursor: pointer;" title="<jecs:locale key="button.edit"/>"/> 
		</jecs:power>
		</c:if>
		<c:if test="${jpoMemberOrder.productType=='LC' }">
   		<jecs:power powerCode="editPoMemberMFOrderLC">
							<img src="<c:url value='/images/newIcons/pencil_16.gif'/>" 
								onclick="javascript:window.location='editJpoMemberMFOrderLC.html?strAction=editPoMemberMFOrderLC&moId=${jpoMemberOrder.moId}'"
								style="cursor: pointer;" title="<jecs:locale key="button.edit"/>"/> 
		</jecs:power>
		</c:if>
					</c:if>
...
<script type="text/javascript">
   ...
   function editJpoMemberMFOrder(moId){
   		<jecs:power powerCode="editPoMemberMFOrder">
					window.location="editJpoMemberMFOrder.html?strAction=editPoMemberMFOrder&moId="+moId;
			</jecs:power>
		}
