...
<jecs:power powerCode="deletePoMemberIROrder">
				<input type="submit" class="button" name="delete" onclick="bCancel=true;return confirmDelete(this.form,'PoMemberIROrder')" value="<jecs:locale key="operation.button.delete"/>" />
</jecs:power>
</c:if>

<input type="button" class="button" name="back"  onclick="window.location.href='jpoMemberIROrders.html?needReload=1'" value="<jecs:locale key="operation.button.return"/>" />
...
<form:form commandName="jpoMemberOrder" method="post" action="editJpoMemberIROrder.html" onsubmit="return onSubmitCheck(this);" id="jpoMemberOrderForm">
...
<c:if test='${param.strAction=="editPoMemberIROrder"}'>
    <tr><th>
    ...
    <td align="left">
    	...
    	<c:if test='${param.strAction=="addPoMemberIROrder"}'>
...
<c:if test="${sta.count!=1}">
     <div id="tabCot_product_${sta.count}" class="tabCot"  style="display: none;">
        ...
        <td>${item.fpPrice}</td>
          <td>${item.fpPv}</td>
          <td>
            <input type='text' class='text medium' name='qty' value='0' size='3' onchange="set_amt(this,'${item.uniNo }','${item.jpmProduct.productNo}');" id=qty<c:out value="${item.uniNo }" />>
