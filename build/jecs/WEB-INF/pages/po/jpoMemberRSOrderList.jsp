...
<form action="jpoMemberRSOrders.html" method="get" name="searchForm"
	id="searchForm">
	<div class="searchBar">
			...
			<jecs:power powerCode="addPoMemberRSOrder">
				<c:if test="${sessionScope.sessionLogin.userType=='C'}">
					<button name="add" class="btn btn_ins" type="button"
						onclick="location.href='<c:url value="/editJpoMemberRSOrder.html?strAction=addPoMemberRSOrder"/>'">
...
<ec:table tableId="jpoMemberOrderListTable" items="jpoMemberOrderList"
	...
	action="${pageContext.request.contextPath}/jpoMemberRSOrders.html"
			...
			<c:if
				...
				<jecs:power powerCode="editPoMemberRSOrder">
					<img src="<c:url value='/images/newIcons/pencil_16.gif'/>"
						onclick="javascript:editJpoMemberRSOrder('${jpoMemberOrder.moId}')"
...
<script type="text/javascript">
	function editJpoMemberRSOrder(moId) {
		<jecs:power powerCode="editPoMemberRSOrder">
		window.location = "editJpoMemberRSOrder.html?strAction=editPoMemberRSOrder&moId="
				+ moId;
		</jecs:power>
	}
</script>
