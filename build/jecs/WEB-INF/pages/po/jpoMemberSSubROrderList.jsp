...
<form action="jpoMemberSSubROrders.html" method="get" name="searchForm"
	id="searchForm">
	<div class="searchBar">
			...
			<jecs:power powerCode="addPoMemberSSubROrder">
				...
				<c:if test="${sessionScope.sessionLogin.userType=='C'}">
					<button name="add" class="btn btn_ins" type="button"
						onclick="location.href='<c:url value="/editJpoMemberSSubROrder.html?strAction=addPoMemberSSubROrder"/>'">
				...
				<c:if test="${sessionScope.sessionLogin.userType!='C'}">
					<c:if
						...
						<c:if test="${sessionScope.sessionLogin.companyCode=='CN' }">
							<button name="add" class="btn btn_ins" type="button"
								onclick="location.href='<c:url value="/editJpoMemberSSubROrderLC.html?strAction=addPoMemberSSubROrderLC"/>'">
								<jecs:locale key="member.new.num" />
							</button>
						</c:if>
						<c:if test="${sessionScope.sessionLogin.companyCode!='CN' }">
...
<ec:table tableId="jpoMemberOrderListTable" items="jpoMemberOrderList"
	...
	action="${pageContext.request.contextPath}/jpoMemberSSubROrders.html"
			...
			<c:if
				...
				<c:if test="${jpoMemberOrder.productType!='LC' }">
					<jecs:power powerCode="editPoMemberSSubROrder">
						<img src="<c:url value='/images/newIcons/pencil_16.gif'/>"
							onclick="javascript:editJpoMemberSSubROrder('${jpoMemberOrder.moId}')"
				...
				<c:if test="${jpoMemberOrder.productType=='LC' }">
					<jecs:power powerCode="editPoMemberSSubROrderLC">
						<img src="<c:url value='/images/newIcons/pencil_16.gif'/>"
							onclick="javascript:window.location='editJpoMemberSSubROrderLC.html?strAction=editPoMemberSSubROrderLC&moId=${jpoMemberOrder.moId}'"
							style="cursor: pointer;" title="<jecs:locale key="button.edit"/>" />
					</jecs:power>
				</c:if>
			</c:if>
...
<script type="text/javascript">
	function editJpoMemberSSubROrder(moId) {
		<jecs:power powerCode="editPoMemberSSubROrder">
		window.location = "editJpoMemberSSubROrder.html?strAction=editPoMemberSSubROrder&moId="
				+ moId;
		</jecs:power>
	}
</script>
