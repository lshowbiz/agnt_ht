...
<c:if
	...
	<form action="jpoMemberFOrders.html" method="get" name="searchForm"
		id="searchForm">
				...
				<jecs:power powerCode="addPoMemberFOrder">
					<c:if test="${sessionScope.sessionLogin.userType=='C'}">
						<button name="add" class="btn btn_ins" type="button"
							style="width: auto"
							onclick="location.href='<c:url value="/editJpoMemberFOrder.html?strAction=addPoMemberFOrder"/>'">
					...
					<c:if test="${sessionScope.sessionLogin.userType!='C'}">
						<c:if
							...
							<c:if test="${sessionScope.sessionLogin.companyCode=='CN' }">
								<button name="add" class="btn btn_ins" type="button"
									style="width: auto"
									onclick="location.href='<c:url value="/jpoMemberFOrdereLevel.html?strAction=addPoMemberFOrderLevel&funType=LC"/>'">
									<jecs:locale key="member.new.num" />
								</button>
							</c:if>
							<c:if test="${sessionScope.sessionLogin.companyCode!='CN' }">
						...
						<c:if
							...
							<button name="add" class="btn btn_ins" type="button"
								style="width: auto"
								onclick="location.href='<c:url value="/jpoMemberFOrdereLevel.html?strAction=addPoMemberFOrderLevel"/>'">
								<jecs:locale key="member.new.num" />
							</button>
						</c:if>
					</c:if>
...
--%>
					...
					<c:if
						...
						<c:if test="${jpoMemberOrder.productType!='LC' }">
							<jecs:power powerCode="editPoMemberFOrder">
								<img src="<c:url value='/images/newIcons/pencil_16.gif'/>"
									onclick="javascript:editJpoMemberFOrder('${jpoMemberOrder.moId}')"
						...
						<c:if test="${jpoMemberOrder.productType=='LC' }">
							<jecs:power powerCode="editPoMemberFOrderLC">
								<img src="<c:url value='/images/newIcons/pencil_16.gif'/>"
									onclick="javascript:window.location='editJpoMemberFOrderLC.html?strAction=editPoMemberFOrderLC&moId=${jpoMemberOrder.moId}'"
									style="cursor: pointer;"
									title="<jecs:locale key="button.edit"/>" />
							</jecs:power>
						</c:if>
   ...
   function editJpoMemberFOrder(moId){
   		<jecs:power powerCode="editPoMemberFOrder">
					window.location="editJpoMemberFOrder.html?strAction=editPoMemberFOrder&moId="+moId;
			</jecs:power>
		}
