...
with(tree){
	...
	<c:if test="${not empty jsysResources}">
		<c:forEach items="${jsysResources}" var="row">
			...
			N["${parentId}_${row.resId}"]="T:<jecs:locale key="${row.resName}"/>;icon:${menuIcon};folderIcon:${folderIcon};C:doSelectSysModule('${row.resId}','<jecs:locale key="${row.resName}"/>');";
		</c:forEach>
	</c:if>
}
tree.wordLine = false;
...
tree.focus('${param.resId}');

function doSelectSysModule(resId, resName){
	var obj=new Object();
	obj.resId=resId;
	obj.resName=resName;
