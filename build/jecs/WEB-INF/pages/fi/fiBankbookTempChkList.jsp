...
<form action="fiBankbookTempsChk.html" method="get"
	...
	<div class="searchBar">
		...
		<div class="new_searchBar">
			...
			<input name="createrName" type="text" value="${param.createrName }"
				size="10" />
			<input type="hidden" name="strAction" value="listFiBankbookTempsChkJJ" />
		</div>
		<div class="new_searchBar">
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<button name="search" class="btn btn_sele" type="submit">
				<jecs:locale key="operation.button.search"/>
			</button>
			<jecs:power powerCode="verifyFiBankbookTempJJ">
				<button name="verify1" class="btn btn_ins" type="button" onclick="verify($('fiBankbookTempSearchForm'))">
					<jecs:locale key="button.verify"/>
				</button>
			</jecs:power>
			<jecs:power powerCode="doNotDealFiBankbookTempJJ">
				<button name="search" class="btn btn_ins" type="button" onclick="doNotDeal($('fiBankbookTempSearchForm'))">
					<jecs:locale key="button.not.deal"/>
				</button>
			</jecs:power>
			<jecs:power powerCode="notVerifiedFiBankbookTempJJ">
				<button name="search" class="btn btn_ins" type="button" onclick="notVerified($('fiBankbookTempSearchForm'))">
...
<form action="fiBankbookTempsChk.html" method="get"
	name="fiBankbookTempForm" id="fiBankbookTempForm">
	<ec:table tableId="fiBankbookTempListTable" items="fiBankbookTempList"
		...
		<ec:row>
			...
			${fiBankbookTemp.sysUser.userName }
		</ec:column>
			<ec:column property="moneyType" title="fiBankbookTemp.moneyType"
				...
				<jecs:code listCode="fibankbooktemp.moneytype"
					value="${fiBankbookTemp.moneyType}" />
			</ec:column>
			<ec:column property="dealType" title="fiBankbookTemp.dealType"
				cell="com.joymain.jecs.util.ectable.EcTableCell">
				<jecs:code listCode="fibankbooktemp.dealtype"
					value="${fiBankbookTemp.dealType}" />
			</ec:column>
			<ec:column property="notes" title="bdBounsDeduct.summary"
				sortable="false">
				<jecs:substr key="${fiBankbookTemp.notes }" length="15"/>
		</ec:column>
			<ec:column property="description" title="bdBounsDeduct.description"
				sortable="false">
				<jecs:substr key="${fiBankbookTemp.description }" length="15"/>
		</ec:column>
			<ec:column property="addMoney" title="fibankbooktemp.dealtype.A"
				styleClass="numberColumn"
				cell="com.joymain.jecs.util.ectable.EcTableCell">
				<c:if test="${fiBankbookTemp.dealType=='A'}">
					<fmt:formatNumber value="${fiBankbookTemp.money}" type="number"
						pattern="###,###,##0.00" />
				</c:if>
				<c:if test="${fiBankbookTemp.dealType=='D'}">0.00</c:if>
			</ec:column>
			<ec:column property="decMoney" title="fibankbooktemp.dealtype.D"
				styleClass="numberColumn"
				cell="com.joymain.jecs.util.ectable.EcTableCell">
				<c:if test="${fiBankbookTemp.dealType=='A'}">0.00</c:if>
				<c:if test="${fiBankbookTemp.dealType=='D'}">
					<fmt:formatNumber value="${fiBankbookTemp.money}" type="number"
						pattern="###,###,##0.00" />
				</c:if>
			</ec:column>
...
function doNotDeal(theForm){
	...
	theForm.strAction.value="doNotDealFiBankbookTempJJ";
	if(isFormPosted()){
		theForm.submit();
	}
}
...
function notVerified(theForm){
	...
	theForm.strAction.value="notVerifiedFiBankbookTempJJ";
	if(isFormPosted()){
		theForm.submit();
	}
}
...
function verify(theForm){
	...
	theForm.strAction.value="verifyFiBankbookTempJJ";
	if(isFormPosted()){
		theForm.submit();
	}
}
