...
<title><jecs:locale key="jfiPay99billDetail.title" />
</title>
<content tag="heading">
<jecs:locale key="jfiPay99billDetail.heading" />
</content>
<script language="javascript" src="scripts/validate.js"></script>
<script type="text/javascript" src="<c:url value='/dwr/util.js'/>"></script>
<script type="text/javascript" src="<c:url value='/dwr/engine.js'/>"></script>
<script type="text/javascript"
	src="<c:url value='/dwr/interface/bill99Util.js'/>"></script>
<script type="text/javascript"
	src="<c:url value='/dwr/interface/Bill99.js'/>"></script>
<c:if test='${empty param.orderId}'>
	<div class="searchBar">
		<c:if test="${param.offline == '1' }">
			<input name="change" class="button" type="button"
				onclick="location.href='jfiPay99bill.html?strAction=fiPay99bill&offline=0'"
				value="<jecs:locale key="jfiPay99bill.online"/>" />
		</c:if>
		<c:if test="${param.offline != '1' }">
			<input name="change" class="button" type="button"
				onclick="location.href='jfiPay99bill.html?strAction=fiPay99bill&offline=1'"
				value="<jecs:locale key="jfiPay99bill.offline"/>" />
		</c:if>
	</div>
</c:if>

<form id="jfiPay99bill" method="GET" action=""
	<c:if test='${empty param.orderId && param.offline == "1"}'>target="_blank" </c:if>
	...
	<input type="hidden" id="payerContactType" name="payerContactType"
		value="" />
	...
	<table class='detail' width="100%">
		<tr>
			...
			<td align="left">
				<input id="payerName" name="payerName" type="text"
					value="${sessionScope.sessionLogin.userName }" size="30" />
			</td>
		</tr>
		<c:if test='${!(empty param.orderId && param.offline == "1")}'>
			<tr>
				...
				<td align="left">
					<jecs:list listCode="bill99.bank.id" name="bankId" id="bankId"
						value="ICBC" defaultValue="ICBC" />
				</td>
			</tr>
		</c:if>
		<c:if test='${empty param.orderId && param.offline == "1"}'>
		...
		<tr>
			...
			<td align="left">
				<c:if test="${empty jpoMemberOrder}">
					<input id="orderAmount1" name="orderAmount1" type="text" value=""
						size="10" onchange="resetOrderAmount()" />
				</c:if>
				<c:if test="${not empty jpoMemberOrder}">
					<input id="orderAmount1" name="orderAmount1" type="text"
						value="${jpoMemberOrder.amount }" size="10" readonly="readonly" />
					<input type="checkbox" id="bankbook" onclick="resetOrderAmount();"
						name="bankbook" value="1" />
					<jecs:locale key="bankbook.pay" />
				</c:if>
				<c:if test="${not empty jpoMemberOrder}">
					<c:if test="${not empty jpoMemberOrder && (coinPay=='true')}">
						<jecs:power powerCode="jfiPayByCoinAndBankbook">
							<input name="change" class="button" type="button"
								onclick="location.href='jfiPayByCoinAndBankbook.html?strAction=jfiPayByCoinAndBankbook&orderId=${param.orderId }'"
								value="<jecs:locale key="jfiPayByCoin.Button"/>" />
						</jecs:power>
					</c:if>
					<jecs:power powerCode="jfiPayByJJ">
						<input name="change" class="button" type="button"
							onclick="location.href='jfiPayByJJ.html?strAction=jfiPayByJJ&orderId=${param.orderId }'"
							value="<jecs:locale key="jfiPayByJJ.Button"/>" />
					</jecs:power>
				</c:if>
			</td>
		</tr>
...
<c:if test="${not empty jpoMemberOrder }">
			...
			<td align="left">
				<c:forEach items="${jpoMemberOrder.jpoMemberOrderFee }" var="pof">
					<c:if test="${pof.feeType=='1' }">
						<jecs:code listCode="feetype" value="${pof.detailType}" />:<fmt:formatNumber
							value="${pof.fee}" type="number" pattern="###,###,##0.00" />
						<br />
					</c:if>
					<c:if test="${pof.feeType=='2' }">
						<jecs:code listCode="handle.fee" value="${pof.detailType}" />:<fmt:formatNumber
							value="${pof.fee}" type="number" pattern="###,###,##0.00" />
						<br />
					</c:if>
				</c:forEach>
	...
	<ec:table tableId="piProductListTable"
		items="jpoMemberOrder.jpoMemberOrderList" var="pi"
		action="${pageContext.request.contextPath}/editPoMemberFOrder.html"
		width="100%" form="poMemberOrderForm" showPagination="false"
		sortable="false" imagePath="./images/extremetable/*.gif">
		<ec:row>
			<ec:column property="jpmProductSale.jpmProduct.productNo"
			...
			<ec:column property="price" title="pd.price"
				...
				<fmt:formatNumber value="${pi.price}" type="number"
					pattern="###,###,##0.00" />
			</ec:column>
			<ec:column property="pv" title="busi.direct.pv"
				styleClass="numberColumn" />
...
function actionPost(){
	if($('orderAmount1').value < 0 || !isUnsignedNumeric($('orderAmount1').value) || !isUnsignedMoney($('orderAmount1').value)){
		$('orderAmount1').value = 0;
		return ;
	}
	<c:if test='${empty param.orderId}'>
