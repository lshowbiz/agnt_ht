...
<title><jecs:locale key="fiBillAccountWarningList.title"/></title>
<content tag="heading"><jecs:locale key="fiBillAccountWarningList.heading"/></content>
<meta name="menu" content="FiBillAccountWarningMenu"/>

<form action="fiBillAccountWarningsChk.html" method="get" name="fiBillAccountWarningSearchForm" id="fiBillAccountWarningSearchForm">

	<input name="search" class="button" type="button" onclick="location.href='<c:url value="/fiBillAccountWarningsChk.html"/>?'" value="<jecs:locale key="operation.button.refresh"/>"/>
	<input name="search" class="button" type="button" onclick="location.href='<c:url value="/fiBillAccountWarningsChk.html"/>?opt=1'" value="<jecs:locale key="operation.button.recom"/>"/>
	<input type="button" class="button" style="margin-right: 5px" onclick="location.href='<c:url value="/fiBillAccounts.html"/>'" value="<jecs:locale key="button.back"/>"/>
...
<table class='detail' width="100%">
		...
		<c:forEach items="${fiBillAccountWarningList}" var="mfiBillAccounts">
			<tr align="right" <c:if test="${(mfiBillAccounts.income_limit - mfiBillAccounts.now_total_amount)<=0}">style='background-color:#FF9933'</c:if> >
				<td>
				...
				<td>${mfiBillAccounts.bill_account_code}</td>
				<td>${mfiBillAccounts.account_name}</td>

				<td>${mfiBillAccounts.income_limit}</td>
				<td>${mfiBillAccounts.now_total_amount}</td>
				<td>
					<c:set var="a" value="${mfiBillAccounts.income_limit}"/>
					<c:set var="b" value="${mfiBillAccounts.now_total_amount}"/>
					<c:if test="${(mfiBillAccounts.income_limit - mfiBillAccounts.now_total_amount)<10000}">
