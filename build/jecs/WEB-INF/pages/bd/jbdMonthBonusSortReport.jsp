...
<form:form commandName="bdPeriod" method="post" action="jbdMonthBonusSortReport.html" onsubmit="return validateOthers(this)" id="bdPeriodForm">
	...
	<div class="searchBar">
				<jecs:power powerCode="jbdMonthBonusSortReport">	
				<input type="submit" name="submit" value="<jecs:locale key="button.report"/>" class="button"/>
				</jecs:power>
				<input type="hidden" name="strAction" value="jbdMonthBonusSortReport"/>
	</div>
	...
	<form:hidden path="wid"/>
		...
		<tr>
			...
			<td >
				<input type="text" name="formatedWeekStart" id="formatedWeekStart" size="8"/>-
				<input type="text" name="formatedWeekEnd" id="formatedWeekEnd" size="8"/>
...
<input name="amount" type="text" value="${param.amount }" size="12"/>	
			</td>
		</tr>
...
function validateOthers(theForm){
	if(!isUnsignedInteger(theForm.formatedWeekEnd.value) || theForm.formatedWeekEnd.value.length!=6 || !isUnsignedInteger(theForm.formatedWeekStart.value) || theForm.formatedWeekStart.value.length!=6){
		alert("<jecs:locale key="week.isError"/>");
		theForm.formatedWeekStart.focus();
		return false;
	}
	if(!isUnsignedInteger(theForm.amount.value)){
