...
<c:if test="${not empty bdlist}">
	...
	<c:if test="${status.count == 1}">
		tree.N["0_<c:out value="${curBd.user_code}"/>"] = "T:<font color='#0066CC'><c:out value="${curBd.user_code} - ${curBd.user_name}"/> - [<jecs:code listCode="member.level.new" value="${curBd.member_level}"/>] - "+${curBd.com_group_pv}+" -  [<fmt:formatDate value="${curBd.check_date }" pattern="yyyy-MM-dd"/>]  -  <c:if test="${curBd.state=='1'}">授权</c:if><c:if test="${curBd.state=='0'}">停权</c:if> </font>;";
	</c:if>
	<c:if test="${status.count >= 1}">
		tree.N["<c:out value="${curBd.user_code}"/>_<c:out value="${curBd.link_no}"/>"] = "T:<font color='#0066CC'><c:out value="${curBd.user_code} - ${curBd.user_name}"/>  - [<jecs:code listCode="member.level.new" value="${curBd.member_level}"/>] - "+${curBd.com_group_pv}+" -  [<fmt:formatDate value="${curBd.check_date }" pattern="yyyy-MM-dd"/>]   -  <c:if test="${curBd.state=='1'}">授权</c:if><c:if test="${curBd.state=='0'}">停权</c:if> </font> ;";
	</c:if>
	</c:forEach>
</c:if>
