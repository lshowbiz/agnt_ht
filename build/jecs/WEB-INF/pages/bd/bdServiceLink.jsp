...
<div class="new_searchBar">	
	...
	<input type="text" name="formatedWeek" id="formatedWeek" size="8" value="${param.formatedWeek}" style="width: 70px;"/>
	</div>
<div class="new_searchBar">	
...
<div class="new_searchBar">	
	...
	<input type="text" name="showLevel" value="${param.showLevel }" style="width: 70px;"/>
</div>
...
<c:if test="${not empty bdlist}">
	...
	<c:forEach items="${bdlist}" var="curBd" varStatus="status">
		...
		<c:if test="${status.count == 1}">
			tree.N["0_<c:out value="${curBd.user_code}"/>"] = "T:<font color=#0066CC><c:out value="${curBd.user_code} - ${curBd.user_name}"/>  - [<jecs:code listCode="grade.type" value="${curBd.grade_type}"/>] - [<c:out value="${curBd.maxpart_pv}"/>] - [<fmt:formatDate value="${curBd.check_date }" pattern="yyyy-MM-dd"/>]  -  <c:if test="${curBd.state=='1'}">授权</c:if><c:if test="${curBd.state=='0'}">停权</c:if> </font> ;";
		</c:if>
		<c:if test="${status.count != 1}">
			
			tree.N["<c:out value="${curBd.link_no}"/>_<c:out value="${curBd.user_code}"/>"] = "T:<font color=#0066CC><c:out value="${curBd.user_code} - ${curBd.user_name}"/>  - [<jecs:code listCode="grade.type" value="${curBd.grade_type}"/>] - [<c:out value="${curBd.maxpart_pv}"/>] - [<fmt:formatDate value="${curBd.check_date }" pattern="yyyy-MM-dd"/>]   -  <c:if test="${curBd.state=='1'}">授权</c:if><c:if test="${curBd.state=='0'}">停权</c:if> </font> ;";
