...
<c:if test="${isFinished!=true}">
	<form:form commandName="bdBounsDeductAll" method="post" action=""
		onsubmit="return validateOthers(this)" id="bdBounsDeductAllForm"
		enctype="multipart/form-data">

		<spring:bind path="bdBounsDeductAll.*">
		...
		<table class='detail' width="70%">
			...
			<tbody class="window">				
				...
				<tr class="edit_tr">
					...
					<td colspan="3" >
					 	第一列：财年；第二列：会员编号；第三列：积分；第四列：取出 A 存入 D 取出；第五列：备注；
					</td>			
				</tr>
				<tr>
					<td class="command" colspan="4" align="center">
						<jecs:power powerCode="jbdTravelPointLogAllImport">
							<input type="submit" class="btn btn_sele" name="import"
								onclick="bCancel=false"
								value="<jecs:locale key="button.import"/>" />
						</jecs:power>
						<input type="hidden" name="strAction"
							value="jbdTravelPointLogAllImport" />					
					</td>
				</tr>
			</tbody>
		</table>
