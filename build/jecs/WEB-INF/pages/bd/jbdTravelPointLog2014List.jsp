...
<jecs:title key="pd.createTime"/>
	 <input id="startTime" name="startTime" type="text" size="8" maxlength="10" value="${param.startTime }"/>
		...
		<jecs:power powerCode="addJbdTravelPointLog2014">
			    <input type="button" class="button" style="margin-right: 5px"
			        onclick="location.href='<c:url value="/editJbdTravelPointLog2014.html"/>?strAction=addJbdTravelPointLog2014'"
...
<ec:table 
	tableId="jbdTravelPointLog2014ListTable"
	items="jbdTravelPointLog2014List"
	var="jbdTravelPointLog2014"
	action="${pageContext.request.contextPath}/jbdTravelPointLog2014s.html"
	width="100%"
	...
	rowsDisplayed="20" sortable="true" imagePath="./images/extremetable/*.gif">
				...
				<jecs:power powerCode="jbdTravelPointLog2014sExport">
				
	<ec:exportXls fileName="jbdTravelPointLog2014s.xls"></ec:exportXls>
				</jecs:power>
		...
		<ec:column property="usePoint" title="fibankbooktemp.dealtype.A" cell="com.joymain.jecs.util.ectable.EcTableCell">
			<c:if test="${jbdTravelPointLog2014.dealType=='A'}">${jbdTravelPointLog2014.usePoint}</c:if>
			<c:if test="${jbdTravelPointLog2014.dealType=='D'}">0</c:if>
		</ec:column>
		<ec:column property="usePoint" title="fibankbooktemp.dealtype.D" cell="com.joymain.jecs.util.ectable.EcTableCell">
			<c:if test="${jbdTravelPointLog2014.dealType=='D'}">${jbdTravelPointLog2014.usePoint}</c:if>
			<c:if test="${jbdTravelPointLog2014.dealType=='A'}">0</c:if>
		</ec:column>
